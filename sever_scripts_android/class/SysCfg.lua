...
local SysCfg = class("SysCfg")
	...
	function SysCfg:ctor()
		...
		--self._version_control = -1 --正式版 客户端版本控制
		--self._debug_version_control = -1 --debug版 客户端版本控制
		
		--self._shop_control = -1 --正式版 商店版本控制
	...
	function SysCfg:Init()
		...
		--self._version_control = 0 --正式版 客户端版本控制
		--self._debug_version_control = 0 --debug版 客户端版本控制
		
		--self._shop_control = 0 --正式版 商店版本控制
	...
	function SysCfg:Update()
		...
		--[[
		if self._statisticsTime > -1 and timeNow - self._statisticsTime > 300 then
			self._statisticsTime = timeNow
			self:_DBUpdateBattleInfoStatistics()
		end
		]]
		
		if self._syscfgTime > -1 and timeNow - self._syscfgTime > 60 then
			self._syscfgTime = timeNow
	...
	--获取正式版 客户端版本控制
	function SysCfg:GetVersionControl()
		return self._version_control
	end
	
	--获取debug版 客户端版本控制
	function SysCfg:GetDebugVersionControl()
		return self._debug_version_control
	end
	
	--获取正式版 商店版本控制
	function SysCfg:GetShopControl()
		return self._shop_control
	end
	]]
	
	
	--获取指定key的配置
	function SysCfg:GetConfgByKey(key)
		if (type(self._configDic) == "table") then
			return self._configDic[key] or 0
		else
			return 0
		end
	end
	...
	--[[
	--获取玩家对应的客户端版本
	function SysCfg:GetUserVersionControl(udbid)
		--local ret = self._version_control
		local ret = self:GetConfgByKey("version_control")
		local user = hGlobal.uMgr:FindUserByDBID(udbid)
		if user then
			if user:IsTesters() then
				--ret = self._debug_version_control
				self:GetConfgByKey("debug_version_control")
			end
		end
		return ret
	end
	...
	function SysCfg:GetUserPvpControl(udbid)
		...
		local ret = self:GetConfgByKey("pvp_control")
		local user = hGlobal.uMgr:FindUserByDBID(udbid)
		if user then
			if user:IsTesters() then
				--ret = self._debug_pvp_control
				self:GetConfgByKey("debug_pvp_control")
			end
		end
		return ret
	end
	...
	--获取玩家对应的商店版本
	function SysCfg:GetUserShopControl(udbid)
		--local ret = self._shop_control
		local ret = self:GetConfgByKey("shop_control")
		local user = hGlobal.uMgr:FindUserByDBID(udbid)
		if user then
			--if user:IsTesters() then
			--	ret = self._shop_control
			--end
		end
		return ret
	end
		...
		if err == 0 then
			for n = 1, #tTemp do
				...
				if readAll or ((not readAll) and stats == 0) then
					...
					elseif key == "version_control" then
						self._debug_pvp_control = content
					end
					]]
					self._configDic[key] = content
				end
	...
	--清除玩家数据
	function SysCfg:ClearData(uid, rid)
		local sCmd = ""
		local result = 0
		
		--清除t_cha数据
		local sUpdate = string.format("UPDATE `t_cha` SET `snapshotid` = 0, `bag` = NULL, `card` = NULL, `skill` = NULL, `map` = NULL, `material` = NULL, `log` = NULL, `chestInfo` = NULL, `heroInfo` = NULL, `tacticInfo` = NULL, `inventoryInfo` = NULL, `levelInfo` = NULL, `treasure` = NULL, `treasure_attr` = NULL WHERE `uid` = %d", uid)
		xlDb_Execute(sUpdate)
		
		--清除t_pvp_user数据
		local sUpdate = string.format("UPDATE `t_pvp_user` SET `heroInfo` = NULL, `chestInfo` = NULL, `tacticInfo` = NULL WHERE `uid` = %d", uid)
		xlDb_Execute(sUpdate)
		
		--清除装备
		local sUpdate = string.format("UPDATE `t_user_redequip` SET `deleteflag` = 1 WHERE `uid` = %d AND `rid` = %d", uid, rid)
		xlDb_Execute(sUpdate)
		
		--清除玩家的全部红装缓存数据
		hGlobal.redEquipUserCacheMgr:ClearRedEquipMgr(uid, rid)
		
		--插入订单
		local shopitemId = 99
		local itemId = hVar.tab_shopitem[shopitemId].itemID
		hGlobal.userCoinMgr:DBUserPurchase(uid, rid, itemId, 1, 0, 0, "")
		
		--操作成功
		result = 1
		sCmd = tostring(result) .. ";"
