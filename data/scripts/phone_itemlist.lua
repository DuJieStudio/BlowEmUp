			...
			if (type(itemList[i]) == "table") and (itemList[i][1]) then
				...
				if tempID then
					...
					_childUI["giftItem_image_bg_"..i] = hUI.image:new({
						parent = _parent,
						model = hVar.ITEMLEVEL[itemLv].BACKMODEL,
						x = _itemX + (idx-1)%8*_itemOffX,
						y = _itemY - math.ceil((idx-8)/8)*itemOffY,
						w = _itemW,
						h = _itemW,
					})
...
table.sort(ttt, function(t1, t2)
	...
	local itemLv1 = tItem[t1[1]].itemLv
	local itemLv2 = tItem[t2[1]].itemLv
	val1 = val1 + itemLv1
	val2 = val2 + itemLv2
...
for i = 1, (40 - ornamentsyNum), 1 do
	table.insert(ttt, lastOrnamentsIdx + 1, {})
end

if hGlobal.UI.PlayerGiftBagFrm then
	hGlobal.UI.PlayerGiftBagFrm:del()
end
