...
local on_receive_limittime_gift_order_back_event = hApi.DoNothing --收到限时礼包充值返回
...
--充值礼包操作面板
hGlobal.UI.InitChargeMoneyFrm_Gift = function(mode)
	local tInitEventName = {"LocalEvent_InitInAppPurchaseGiftFrm_Gift", "__ShowChargeMoneyFrm"}
	if (mode ~= "reload") then
	...
	--不重复创建
	if hGlobal.UI.PhoneChargeMoneyFrm_Gift then --充值面板
		hGlobal.UI.PhoneChargeMoneyFrm_Gift:del()
		hGlobal.UI.PhoneChargeMoneyFrm_Gift = nil
	end
	...
	--[[
	--取消监听充值界面通知事件
	hGlobal.event:listen("LocalEvent_InitInAppPurchaseGiftFrm_Gift", "__ShowChargeMoneyFrm", nil)
	--取消监听关闭宝箱界面事件
	...
	hApi.clearTimer("__CHARGE_MONEY_GIFT_UPDATE__")
	
	--创建充值礼包操作面板
	hGlobal.UI.PhoneChargeMoneyFrm_Gift = hUI.frame:new(
	{
		x = BOARD_POS_X,
		y = BOARD_POS_Y,
		z = 500-1,
	...
	local _frm = hGlobal.UI.PhoneChargeMoneyFrm_Gift
	...
	OnClickPageBtn = function(pageIndex)
		...
		--移除事件监听：收到1元档商品信息返回
		hGlobal.event:listen("GetPurchaseGiftList", "__LimitTimeGift", nil)
		--移除事件监听：收到1元档商品充值返回
		hGlobal.event:listen("localEvent_LimitTimeGiftOrderRet", "__LimitTimeGiftOrderBack", nil)
	...
	OnCreateChargeMoneyFrame = function(pageIdx)
		...
		--添加事件监听：收到1元档商品信息返回
		hGlobal.event:listen("GetPurchaseGiftList", "__LimitTimeGift", on_receive_limittime_gift_event)
		--添加事件监听：收到1元档商品充值返回
		hGlobal.event:listen("localEvent_LimitTimeGiftOrderRet", "__LimitTimeGiftOrderBack", on_receive_limittime_gift_order_back_event)
		...
		_frmNode.childUI["BillboardTitlel"] = hUI.label:new({
			...
			border = 1,
			RGB = {255, 255, 255},
			text = hVar.tab_string["__TEXT_PAGE_GIFT"], --"礼包"
		})
		...
		_frmNode.childUI["BillboardButton3"] = hUI.button:new({
			...
			h = 90,
			z = 100,
			label = {x = 0, y = 0, font = hVar.FONTC, align = "MC", border = 1, text = hVar.tab_string["__TEXT_PAGE_GIFT"], size = 36, width = 300, RGB = {255, 255, 0,},}, --"礼包"
			--scale = 0.8,
			dragbox = _frm.childUI["dragBox"],
			scaleT = 0.95,
			code = function()
		...
		if iChannelId < 100 then
			...
			if (current_Iap_max_num == 0) then
				--苹果需要先查询档位，再查询1元档商品
				local iType = 1
				xlRequestIapList(iType)
			else
				--geyachao: 应王总要求，如果有缓存充值条目就不查询了
				--收到限时商品信息回调
				on_receive_limittime_gift_event(0, current_iIapType, 0, 0, current_Iap_max_num, current_IapList)
			end
			]]
			--[[
			--苹果需要先查询档位，再查询1元档商品
	...
	--函数：收到限时商品信息回调
	on_receive_limittime_gift_event = function(iErrCode, iIapType, iUid, iRid, iItemNum, list)
		local _frm = hGlobal.UI.PhoneChargeMoneyFrm_Gift
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		
		--print("on_receive_limittime_gift_event", iErrCode, iIapType, iUid, iRid, iItemNum, list)
		...
		if (iErrCode == 0) then --0:成功
			--存储数量
			current_Iap_max_num = iItemNum
			current_IapList = list
			current_iIapType = iIapType
			
			--找到1元档商品
			for i = 1, iItemNum, 1 do
				local iGiftId = list[i].iGiftId or 0
				local iGiftMoney = list[i].iGiftMoney or 0
				local iCount = list[i].iCount or 0 --已购买次数
				local iMaxCount = list[i].iMaxCount or 1 --最大购买次数
				local szGiftInfo = list[i].szGiftInfo or "" --礼包描述信息
				local szGiftPrize = list[i].szGiftPrize or "" --奖励表
				...
				--解析奖励表
				local reward = {}
				for p = 1, #first_buy, 1 do
					local tGiftPrize = first_buy[p]
					--print("tGiftPrize=", tGiftPrize)
					local tCmd = hApi.Split(tGiftPrize.detail,";")
					for i = 2, #tCmd, 1 do
						if tCmd[i] and (tCmd[i] ~= "") then
							local rewardT = hApi.Split(tCmd[i], ":")
							local t = {}
							reward[#reward+1] = t
							t[1] = tonumber(rewardT[1]) or 0
							t[2] = tonumber(rewardT[2]) or 0
							t[3] = tonumber(rewardT[3]) or 0
							t[4] = tonumber(rewardT[4]) or 0
							--print(i, t[1], t[2], t[3], t[4])
						end
					end
				end
				...
				_frmNode.childUI["AchievementNode" .. i].childUI["ConfimBtn"].childUI["money"] = hUI.label:new({
					...
					width = 300,
					border = 1,
					text = string.format(hVar.tab_string["cmd_40105"], iGiftMoney),
				})
			...
			if (current_Iap_max_num == 0) then
				...
				_frmNode.childUI["DisabelConnectLabel"] = hUI.label:new({
					...
					size = 26,
					x = PRUCHASE_OFFSET_X + 476,
					y = PRUCHASE_OFFSET_Y - 290 + 6,
					width = 800,
					align = "MC",
					font = hVar.FONTC,
					--text = "连接AppStore失败，请检查您的网络设置。", --language
					text = hVar.tab_string["ios_pruchase_connect"] .. hVar.tab_string["ios_pruchase_pay" .. current_iIapType] .. hVar.tab_string["ios_pruchase_fail"], --language
					border = 1,
				})
	...
	on_spine_screen_event = function()
		...
		--重绘本界面
		hGlobal.UI.InitChargeMoneyFrm_Gift("reload") --测试
		--触发事件，显示充值礼包界面
		hGlobal.event:event("LocalEvent_InitInAppPurchaseGiftFrm_Gift", current_funcCallback)
	end
	...
	OnSelectChargeMoneyButton = function(idxEx)
		...
		if (current_focus_achievementEx_idx > 0) then
			...
			if (current_deal_idx == 0) then
				...
				local listI = current_IapList[current_focus_achievementEx_idx]
				local iGiftId = listI.iGiftId or 0
				local iGiftMoney = listI.iGiftMoney or 0
				local iCount = listI.iCount or 0 --已购买次数
				local iMaxCount = listI.iMaxCount or 1 --最大购买次数
				local szGiftInfo = listI.szGiftInfo or "" --礼包描述信息
				local szGiftPrize = listI.szGiftPrize or "" --奖励表
				local sCfg = "local tmp = {" .. szGiftPrize .. "} return tmp"
				local tGiftPrizeList = assert(loadstring(sCfg))()
				local first_buy = tGiftPrizeList.first_buy --首次充值的奖励列表
				...
				--if (iChannelId == 10002) then --taptap
					...
					local onclickevent_zfb = function()
						...
						--发起1元档充值
						--print(iIapType, iGiftId, type(iIapType), type(iGiftId))
						local iIapType = current_iIapType
						local iChannelId = xlGetChannelId()
						if iChannelId < 100 then
							SendCmdFunc["iap_sale_gift_order_ios"](iIapType, iGiftId)
						else
							SendCmdFunc["iap_sale_gift_order"](iIapType, iGiftId)
						end
					end
					...
					local onclickevent_wx = function()
						...
						if (xlGetChannelVersion() >= 2023010101) then
							...
							--发起1元档充值
							SendCmdFunc["iap_sale_gift_order"](4, iGiftId)
						else
							--弹框，版本太旧
	...
	--函数：收到充值信息列表回调（仅限苹果）
	on_receive_IapList_gift_event = function(list)
		local _frm = hGlobal.UI.PhoneChargeMoneyFrm_Gift
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
	...
	--函数：收到限时商品充值返回
	on_receive_limittime_gift_order_back_event = function(iErrCode)
		...
		--[[
		--冒字
		local msgTitle = "充值返回:" .. tostring(iErrCode) .. "," .. tostring(iIapType) .. "," .. tostring(iUid) .. "," .. tostring(iRid) .. ",\n" .. tostring(iItemId) .. "," .. tostring(sOrderId) .. "," .. tostring(sOrderData) .. "," .. tostring(sPrice)
		...
		if (iErrCode == 0) then --(0:成功)
			...
			--弹框
			--"充值成功！"
			hGlobal.UI.MsgBox(hVar.tab_string["recharge_success"],{
				font = hVar.FONTC,
				ok = function()
					--点确定后跳转邮箱
			...
			--冒字
			local strText = "充值失败！错误码=" .. tostring(iErrCode)
			if (iErrCode == 2) then
				strText = "充值失败！错误的充值类型:" .. tostring(iIapType)
			elseif (iErrCode == 3) then
				strText = "充值失败！错误的礼包类型:" .. tostring(iItemId)
			elseif (iErrCode == 101) then
				strText = "充值失败！礼包[" .. tostring(iItemId) .. "]价格异常！"
			elseif (iErrCode == 102) then
				strText = "您已购买过该礼包，不能重复购买！"
			elseif (iErrCode == 103) then
				strText = "上一个交易正在处理中，请2分钟后再试！"
			end
	...
	OnClosePanel = function()
		--不显示充值面板
		hGlobal.UI.PhoneChargeMoneyFrm_Gift:show(0)
...
--监听充值界面通知事件
hGlobal.event:listen("LocalEvent_InitInAppPurchaseGiftFrm_Gift", "__ShowChargeMoneyFrm", function(callback, bOpenImmediate)
	--print("监听充值界面通知事件",callback, bOpenImmediate)
	hGlobal.UI.InitChargeMoneyFrm_Gift("reload")
	...
	if bOpenImmediate then
		...
		--显示道具界面
		hGlobal.UI.PhoneChargeMoneyFrm_Gift:show(1)
		hGlobal.UI.PhoneChargeMoneyFrm_Gift:active()
		...
		--创建timer，刷新充值滚动
		hApi.addTimerForever("__CHARGE_MONEY_GIFT_UPDATE__", hVar.TIMER_MODE.GAMETIME, 30, function()
...
--[[
--测试代码
if hGlobal.UI.PhoneChargeMoneyFrm_Gift then --删除上一次的充值界面
	hGlobal.UI.PhoneChargeMoneyFrm_Gift:del()
	hGlobal.UI.PhoneChargeMoneyFrm_Gift = nil
end
hGlobal.UI.InitChargeMoneyFrm_Gift("reload") --测试
--触发事件，显示充值界面
hGlobal.event:event("LocalEvent_InitInAppPurchaseGiftFrm_Gift")
]]
