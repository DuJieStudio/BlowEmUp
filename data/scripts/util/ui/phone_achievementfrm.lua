...
--函数部分
--分页1：成就部分
local OnCreateAchievementFrame = hApi.DoNothing --创建成就界面（第1个分页）
...
local on_achievement_receive_info_event = hApi.DoNothing --成就收到成就信息的回调
local on_achievement_takereward_back_event = hApi.DoNothing --收到领取成就奖励的结果回调
local on_spine_screen_event = hApi.DoNothing --横竖屏切换
local on_create_single_achievement_UI = hApi.DoNothing --绘制单条任务数据
local on_create_single_achievement_UI_async = hApi.DoNothing --异步绘制单条任务数据（异步）
...
--local refresh_achievement_timer = hApi.DoNothing --刷新任务倒计时的timer
local refresh_dlcmapinfo_UI_achievement_loop = hApi.DoNothing --刷新任务DLC地图面板滚动的timer
local refresh_async_paint_achievement_loop = hApi.DoNothing --异步绘制任务条目的timer
...
local OnSelectAchievementTakeRewardButton = hApi.DoNothing --点击某个成就领奖按钮处理的逻辑
...
local current_AchievementInfo = {} --任务信息表
local current_async_paint_list_achievement = {} --当前待异步绘制的聊天消息列表
...
local current_medalType = nil --成就类型
local current_funcCallback = nil --关闭后的回调事件
--local current_screenMode = hVar.SCREEN_MODE --当前屏幕横竖屏样式
...
--成就面板
hGlobal.UI.InitMyTaskAchievementFrm = function(mode)
	local tInitEventName = {"LocalEvent_Phone_ShowMyAchievement", "__ShowAchievementFrm",}
	if (mode ~= "reload") then
	...
	--不重复创建
	if hGlobal.UI.PhoneAchievementFrm then --任务操作面板
		hGlobal.UI.PhoneAchievementFrm:del()
		hGlobal.UI.PhoneAchievementFrm = nil
	end
	
	--[[
	--取消监听打开任务、成就、活动界面通知事件
	hGlobal.event:listen("LocalEvent_Phone_ShowMyAchievement", "__ShowAchievementFrm", nil)
	--取消监听关闭宝箱界面事件
	hGlobal.event:listen("clearPhoneChestFrm", "__CloseAchievementFrm", nil)
	]]
	
	--加载资源
	--xlLoadResourceFromPList("data/image/misc/pvp.plist")
	
	--hApi.clearTimer("__ACHIEVEMENT_TIMER_UPDATE__")
	hApi.clearTimer("__ACHIEVEMENT_SCROLL_UPDATE__")
	...
	--创建任务、成就操作面板
	hGlobal.UI.PhoneAchievementFrm = hUI.frame:new(
	{
		x = BOARD_POS_X,
		y = BOARD_POS_Y,
		w = BOARD_WIDTH,
	...
	local _frm = hGlobal.UI.PhoneAchievementFrm
	local _parent = _frm.handle._n
	
	--活动左侧裁剪区域
	local _BTC_PageClippingRect_Task = {190, -10, 530, 564+136, 0,}
	local _BTC_pClipNode_Achievement = hApi.CreateClippingNode(_frm, _BTC_PageClippingRect_Task, 99, _BTC_PageClippingRect_Task[5], "_BTC_pClipNode_Achievement")
	...
	OnClosePanel = function()
		...
		--不显示任务操作面板
		hGlobal.UI.PhoneAchievementFrm:show(0)
	...
	OnClickPageBtn = function(pageIndex)
		...
		if (pageIndex == 1) then --分页1：任务
			--创建成就界面
			OnCreateAchievementFrame(pageIndex)
		end
	...
	--函数：创建成就界面（第2个分页）
	OnCreateAchievementFrame = function(pageIdx)
		local _frm = hGlobal.UI.PhoneAchievementFrm
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		...
		--允许本分页的的clipNode
		hApi.EnableClipByName(_frm, "_BTC_pClipNode_Achievement", 1)
		
		--初始化参数
		--current_DLCMap_max_num_task = 0 --最大的DLC地图面板id
		current_focus_task_idx = 0 --当前选中的任务索引
		current_async_paint_list_achievement = {} --清空异步缓存待绘制内容
		current_AchievementInfo = {} --清空任务信息表
		...
		if (local_srcVer < version_control) then
			...
			_frmNode.childUI["msgboxTitle"] = hUI.label:new({
				parent = _parentNode,
				x = ACTIVITY_OFFSET_X + 360 - 154,
				y = ACTIVITY_OFFSET_Y - 310 + 140 - 50 - 30,
				size = 30,
				font = hVar.FONTC,
				align = "MC",
				width = 400,
			...
			_frmNode.childUI["msgboxContent"] = hUI.label:new({
				parent = _parentNode,
				x = ACTIVITY_OFFSET_X + 360 - 154,
				y = ACTIVITY_OFFSET_Y - 310 + 140 - 110 - 30,
				size = 26,
				font = hVar.FONTC,
				align = "MT",
				width = 660,
		...
		_frmNode.childUI["TaskPageUp_Btn"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				--超过一页的数量才滑屏
				if (#current_AchievementInfo > (TASK_X_NUM * TASK_Y_NUM)) then
					--print("向下滚屏", screenY)
					--向下滚屏
		...
		_frmNode.childUI["TaskDragPanel"] = hUI.button:new({
			...
			codeOnTouch = function(self, touchX, touchY, sus)
				...
				--检测是否点到了某些控件里
				for i = 1, #current_AchievementInfo, 1 do
					...
					if ctrli then
						...
						--检测是否点到了奖励道具
						local tRecordI = current_AchievementInfo[i]
						local medalId = tRecordI.medalId
						local tabAchievement = hVar.tab_medal[medalId] or {}
						local taskReward = tabAchievement.reward or {} --任务奖励
					...
					if ctrli then
						...
						if ctrlI then
							if (ctrlI.data.state == 1) then --按钮可点状态
								...
								if (touchX >= blx) and (touchX <= brx) and (touchY >= bly) and (touchY <= bry) then
									--点击任务领奖按钮
									OnSelectAchievementTakeRewardButton(selected_taskEx_idx)
								end
							end
						end
						
						--检测是否点到了奖励道具
						local tRecordI = current_AchievementInfo[selected_taskEx_idx]
						local medalId = tRecordI.medalId
						local tabAchievement = hVar.tab_medal[medalId] or {}
						local taskReward = tabAchievement.reward or {} --任务奖励
						for r = 1, #taskReward, 1 do
		...
		--只在本分页有效的事件
		--添加事件监听：成就收到成就信息的回调
		hGlobal.event:listen("localEvent_OnReceiveAchievementInfo", "__QueryAchievement", on_achievement_receive_info_event)
		--添加事件监听：收到领取成就奖励的结果回调
		hGlobal.event:listen("localEvent_OnTakeRewardAchievement", "__QueryAchievement", on_achievement_takereward_back_event)
		--添加移除事件监听：横竖屏切换
		hGlobal.event:listen("LocalEvent_SpinScreen", "__SpinScreenAchievement_", on_spine_screen_event)
		
		--只有在本分页才会有的timer
		--hApi.addTimerForever("__ACHIEVEMENT_TIMER_UPDATE__", hVar.TIMER_MODE.GAMETIME, 1000, refresh_achievement_timer)
		--创建timer，刷新任务DLC地图面板滚动
		hApi.addTimerForever("__ACHIEVEMENT_SCROLL_UPDATE__", hVar.TIMER_MODE.GAMETIME, 30, refresh_dlcmapinfo_UI_achievement_loop)
		--异步绘制任务条目的timer
		hApi.addTimerForever("__ACHIEVEMENT_ASYNC_TIMER__", hVar.TIMER_MODE.GAMETIME, 1, refresh_async_paint_achievement_loop)
	...
	--函数：收到成就信息的回调
	on_achievement_receive_info_event = function()
		local _frm = hGlobal.UI.PhoneAchievementFrm
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		--print("on_achievement_receive_info_event")
		...
		for i = 1, #hVar.tab_medalEx, 1 do
			...
			--成就类型一致
			if (condition == current_medalType) or (current_medalType == nil) then
				local state = LuaGetPlayerMedal(medalId) --成就状态(0:未完成 / 1:已达成 / 2:已领取)
				if (state == 1) then
					--只加指定的成就类型（nil表示读全部）
					current_AchievementInfo[#current_AchievementInfo+1] = {medalId = medalId, state = state,}
				end
			end
		end
		--再添加未完成的成就
		...
		for i = 1, #hVar.tab_medalEx, 1 do
			...
			--print(medalId, condition, current_medalType)
			--成就类型一致
			if (condition == current_medalType) or (current_medalType == nil) then
				local state = LuaGetPlayerMedal(medalId) --成就状态(0:未完成 / 1:已达成 / 2:已领取)
				if (state == 2) then
		...
		for i = 1, #current_AchievementInfo, 1 do
			--异步绘制任务单个条目
			current_AchievementInfo[i].index = i
			on_create_single_achievement_UI_async(i, current_AchievementInfo[i])
		...
		--如果没有任务，显示任务已完成
		if (#current_AchievementInfo == 0) then
		...
		--如果未指定成就类型，或者成就类型是星星，那么左侧展示每章的星星
		if (current_medalType == nil) or (current_medalType == hVar.MEDAL_TYPE.starCount) then
			--更新左侧每章星星数
			local totalStar, tStars = LuaGetPlayerStarCountVal()
			
			--总星星图标
			_frmNode.childUI["AchievementTotalStarIcon"] = hUI.image:new({
				parent = _parentNode,
				x = TASK_OFFSET_X - 90,
				y = TASK_OFFSET_Y - 120,
			...
			rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementTotalStarIcon"
			
			--总星星值
			_frmNode.childUI["AchievementTotalStarLabel"] = hUI.label:new({
				parent = _parentNode,
				x = TASK_OFFSET_X - 60,
				y = TASK_OFFSET_Y - 120 - 1,
				size = 26,
				font = "num",
				align = "LC",
				width = 500,
			...
			rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementTotalStarLabel"
			
			--每章的星星数
			for i = 1, #hVar.tab_chapter, 1 do
				local tChapter = hVar.tab_chapter[i]
				local icon = tChapter.icon --图标
				
				--章节图标
				_frmNode.childUI["AchievementChapterBtn_" .. i] = hUI.button:new({ --作为按钮只是为了挂载子控件
					parent = _parentNode,
					x = TASK_OFFSET_X - 70,
					y = TASK_OFFSET_Y - 210 - (i - 1) * 100,
					model = icon,
					w = 76,
					h = 76,
				})
				rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementChapterBtn_" .. i
				
				--章节星星
				_frmNode.childUI["AchievementChapterBtn_" .. i].childUI["star"] = hUI.image:new({
					parent = _frmNode.childUI["AchievementChapterBtn_" .. i].handle._n,
					x = 20,
					y = -20,
					model = "misc/addition/star_light2.png",
					w = 32,
				...
				--章节星星值
				_frmNode.childUI["AchievementChapterBtn_" .. i].childUI["label"] = hUI.label:new({
					parent = _frmNode.childUI["AchievementChapterBtn_" .. i].handle._n,
					x = 40,
					y = -20-4,
					size = 22,
		...
		--如果是科学家类型的成就，左侧展示皮肤获得情况
		if (current_medalType == hVar.MEDAL_TYPE.rescueScientist) then
			--更新左侧总科学家数量
			local totalScientist = LuaGetTankScientistNum()
			
			--总科学家图标
			_frmNode.childUI["AchievementTotalScientistIcon"] = hUI.image:new({
				parent = _parentNode,
				x = TASK_OFFSET_X - 90,
				y = TASK_OFFSET_Y - 120,
			...
			rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementTotalScientistIcon"
			
			--总科学家值
			_frmNode.childUI["AchievementTotalScientistLabel"] = hUI.label:new({
				parent = _parentNode,
				x = TASK_OFFSET_X - 60,
				y = TASK_OFFSET_Y - 120 - 1,
				size = 26,
			...
			rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementTotalScientistLabel"
			
			--依次检测科学家成就皮肤
			local tSientistAchievement = {}
			tSientistAchievement[#tSientistAchievement+1] = {110, "../xlobj/garage_01.png", 0.64, LuaGetScientistAchievnemtn1()} --110:科学家成就1
			tSientistAchievement[#tSientistAchievement+1] = {111, "../xlobj/garage_02.png", 0.64, LuaGetScientistAchievnemtn2()} --112:科学家成就2
			tSientistAchievement[#tSientistAchievement+1] = {112, "../xlobj/garage_03.png", 0.5, LuaGetScientistAchievnemtn3()} --113:科学家成就3
			tSientistAchievement[#tSientistAchievement+1] = {113, "../xlobj/garage_04.png", 0.35, LuaGetScientistAchievnemtn4()} --114:科学家成就4
			for i = 1, #tSientistAchievement, 1 do
				local tInfo = tSientistAchievement[i]
				
				--成就皮肤图标
				_frmNode.childUI["AchievementScientistBtn_" .. i] = hUI.button:new({ --作为按钮只是为了挂载子控件
					parent = _parentNode,
					x = TASK_OFFSET_X - 56,
					y = TASK_OFFSET_Y - 240 - (i - 1) * 150,
					model = tInfo[2],
					scale = tInfo[3],
				})
				rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementScientistBtn_" .. i
				
				--成就皮肤是否已获得
				_frmNode.childUI["AchievementScientistBtn_" .. i].childUI["finishTag"] = hUI.image:new({
					parent = _frmNode.childUI["AchievementScientistBtn_" .. i].handle._n,
					x = 0,
					y = -30,
					model = "UI:FinishTag",
					scale = 1.0,
				})
				_frmNode.childUI["AchievementScientistBtn_" .. i].childUI["finishTag"].handle.s:setRotation(15)
				if (tInfo[4] == 0) then
					_frmNode.childUI["AchievementScientistBtn_" .. i].childUI["finishTag"].handle._n:setVisible(false)
				end
			end
		end
		
		--如果是垃圾堆类型的成就，左侧展示皮肤获得情况
		if (current_medalType == hVar.MEDAL_TYPE.tankDeadthCount) then
			--更新左侧总垃圾堆数量
			local totalTankDeadth = LuaGetTankDeadthNum()
			
			--总科学家图标
			_frmNode.childUI["AchievementTotalTankDeadthIcon"] = hUI.image:new({
				parent = _parentNode,
				x = TASK_OFFSET_X - 90,
				y = TASK_OFFSET_Y - 120,
				model = "misc/task/fail_circle.png",
				w = 42,
				h = 42,
			})
			rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementTotalTankDeadthIcon"
			
			--总垃圾堆值
			_frmNode.childUI["AchievementTotalTankDeadLabel"] = hUI.label:new({
				parent = _parentNode,
				x = TASK_OFFSET_X - 60,
				y = TASK_OFFSET_Y - 120 - 1,
				size = 26,
				...
				width = 500,
				border = 0,
				text = totalTankDeadth,
			})
			rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementTotalTankDeadLabel"
			
			--依次检测科学家成就皮肤
			local tTankDeadthAchievement = {}
			tTankDeadthAchievement[#tTankDeadthAchievement+1] = {115, "../xlobj/remains_01.png", 1.0, LuaGetTankDeadthAchievnemtn1()} --115:垃圾堆成就1
			tTankDeadthAchievement[#tTankDeadthAchievement+1] = {116, "../xlobj/remains_02.png", 1.0, LuaGetTankDeadthAchievnemtn2()} --116:垃圾堆成就2
			tTankDeadthAchievement[#tTankDeadthAchievement+1] = {117, "../xlobj/remains_03.png", 0.9, LuaGetTankDeadthAchievnemtn3()} --117:垃圾堆成就3
			tTankDeadthAchievement[#tTankDeadthAchievement+1] = {118, "../xlobj/remains_04.png", 0.8, LuaGetTankDeadthAchievnemtn4()} --118:垃圾堆成就4
			tTankDeadthAchievement[#tTankDeadthAchievement+1] = {119, "../xlobj/remains_05.png", 0.7, LuaGetTankDeadthAchievnemtn5()} --119:垃圾堆成就5
			for i = 1, #tTankDeadthAchievement, 1 do
				local tInfo = tTankDeadthAchievement[i]
				
				--成就皮肤图标
				_frmNode.childUI["AchievementTankDeadthBtn_" .. i] = hUI.button:new({ --作为按钮只是为了挂载子控件
					parent = _parentNode,
					x = TASK_OFFSET_X - 56,
					y = TASK_OFFSET_Y - 220 - (i - 1) * 120,
					model = tInfo[2],
					scale = tInfo[3],
				})
				rightRemoveFrmList[#rightRemoveFrmList + 1] = "AchievementTankDeadthBtn_" .. i
				
				--成就皮肤是否已获得
				_frmNode.childUI["AchievementTankDeadthBtn_" .. i].childUI["finishTag"] = hUI.image:new({
					parent = _frmNode.childUI["AchievementTankDeadthBtn_" .. i].handle._n,
					x = 0,
					y = -30,
					model = "UI:FinishTag",
					scale = 1.0,
				})
				_frmNode.childUI["AchievementTankDeadthBtn_" .. i].childUI["finishTag"].handle.s:setRotation(15)
				if (tInfo[4] == 0) then
					_frmNode.childUI["AchievementTankDeadthBtn_" .. i].childUI["finishTag"].handle._n:setVisible(false)
				end
			end
		end
		
		--立即触发一次刷新倒计时
		--refresh_achievement_timer()
	...
	--函数：收到领取成就奖励的结果回调
	on_achievement_takereward_back_event = function(result, medalId, rewardResult)
		local _frm = hGlobal.UI.PhoneAchievementFrm
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		...
		if (result == 1) then
			...
			--找到此成就所在的新的索引idx
			local tAchievementInfo = {}
			--先添加已达成的成就
			for i = 1, #hVar.tab_medalEx, 1 do
				local medalId = hVar.tab_medalEx[i] --成就id
				local state = LuaGetPlayerMedal(medalId) --成就状态(0:未完成 / 1:已达成 / 2:已领取)
				if (state == 1) then
					tAchievementInfo[#tAchievementInfo+1] = {medalId = medalId, state = state,}
				end
			end
			--再添加未完成的成就
			for i = 1, #hVar.tab_medalEx, 1 do
			...
			--依次遍历每个成就
			local achievementIdx = 0
			for i = 1, #tAchievementInfo, 1 do
				if (tAchievementInfo[i].medalId == medalId) then --找到了
					achievementIdx = i
					break
				end
			end
			current_focus_achievementEx_idx = achievementIdx
			
			--本地模拟收到成就回调
			on_achievement_receive_info_event()
		end
	end
	
	--函数：异步绘制单条任务数据（异步）
	on_create_single_achievement_UI_async = function(index, tRecordI)
		local _frm = hGlobal.UI.PhoneAchievementFrm
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		
		local medalId = tRecordI.medalId --成就id
		local state = tRecordI.state --成就状态(0:未完成 / 1:已达成 / 2:已领取)
		local tabAchievement = hVar.tab_medal[medalId]
		
		local xn = (index % TASK_X_NUM) --xn
		if (xn == 0) then
			xn = TASK_X_NUM
		end
		local yn = (index - xn) / TASK_X_NUM + 1 --yn
		
		--父控件
		_frmNode.childUI["TaskNode" .. index] = hUI.button:new({
			parent = _BTC_pClipNode_Achievement,
			model = "misc/mask.png",
		...
		--存储异步待绘制消息列表
		current_async_paint_list_achievement[index] = tRecordI
	end
	
	--函数：绘制单条任务数据
	on_create_single_achievement_UI = function(index, tRecordI)
		local _frm = hGlobal.UI.PhoneAchievementFrm
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		--print("medalId=", medalId)
		local medalId = tRecordI.medalId --成就id
		local state = tRecordI.state --成就状态(0:未完成 / 1:已达成 / 2:已领取)
		local tabAchievement = hVar.tab_medal[medalId]
		local taskReward = tabAchievement.reward
		...
		--九宫格
		local s9 = hApi.CCScale9SpriteCreate("data/image/misc/task/mission_bar.png", -2, -2, 505, 103, _frmNode.childUI["TaskNode" .. index])
		--s9:setOpacity(64)
		--s9:setColor(ccc3(244, 244, 244))
		
		--九宫格（选中高亮）
		local s92 = hApi.CCScale9SpriteCreate("data/image/misc/task/mission_bar.png", -2, -2, 505, 103, _frmNode.childUI["TaskNode" .. index])
		...
		_frmNode.childUI["TaskNode" .. index].childUI["TaskIcon"] = hUI.button:new({ --为了换图，所以是按钮
			parent = _frmNode.childUI["TaskNode" .. index].handle._n,
			model = tabAchievement.icon,
			x = -TASK_WIDTH / 2 + 183,
			y = 6,
			w = tabAchievement.width,
			h = tabAchievement.height,
		})
		
		--任务子图标
		if tabAchievement.icon_sub then
			_frmNode.childUI["TaskNode" .. index].childUI["TaskSubIcon"] = hUI.image:new({ --为了换图，所以是按钮
				parent = _frmNode.childUI["TaskNode" .. index].handle._n,
				model = tabAchievement.icon_sub,
				x = -TASK_WIDTH / 2 + 183,
				y = 6,
				w = 64,
				h = 64,
		...
		--成就描述
		local taskDescripe2 = hVar.tab_stringME[medalId] and hVar.tab_stringME[medalId][2] or ("未知成就描述" .. medalId)
		if (current_medalType == hVar.MEDAL_TYPE.starCount) then
			local condition = tabAchievement.conditions[current_medalType]
			taskDescripe2 = string.format(hVar.tab_string["TEXT_StarCount"], condition)
		elseif (current_medalType == hVar.MEDAL_TYPE.rescueScientist) then
			local condition = tabAchievement.conditions[current_medalType]
			taskDescripe2 = string.format(hVar.tab_string["TEXT_RescueScientist"], condition)
		elseif (current_medalType == hVar.MEDAL_TYPE.tankDeadthCount) then
			local condition = tabAchievement.conditions[current_medalType]
			taskDescripe2 = string.format(hVar.tab_string["TEXT_TankDeadthCount"], condition)
		end
		...
		--成就进度
		local maxNum = 0 --总成就点数
		for k, v in pairs(tabAchievement.conditions) do
			if (k == hVar.MEDAL_TYPE.map) or (k == hVar.MEDAL_TYPE.mapCondition) then --
				maxNum = maxNum + 1
			elseif (k == hVar.MEDAL_TYPE.killUT) then --累计击杀N个指定敌人
				maxNum = maxNum + v[2]
			elseif (k == hVar.MEDAL_TYPE.buildTT) then --累计建造N个塔
				maxNum = maxNum + v[2]
			elseif (k == hVar.MEDAL_TYPE.td_wj_001) or (k == hVar.MEDAL_TYPE.td_wj_002) then --无尽关卡1/2战绩
				maxNum = maxNum + v
			else --一般情况
				maxNum = maxNum + v
			end
		...
		local finishNum = 0 --当前完成的成就点数
		for k, v in pairs(tabAchievement.conditions) do
			if (k == hVar.MEDAL_TYPE.map) then --过第N关
				local haveLevel = (LuaGetPlayerMapAchi(v, hVar.ACHIEVEMENT_TYPE.LEVEL) or 0)
				finishNum = finishNum + haveLevel
			elseif (k == hVar.MEDAL_TYPE.mapCondition) or (k == hVar.MEDAL_TYPE.chapterCondition) then
				--finishNum = finishNum + 1
			elseif (k == hVar.MEDAL_TYPE.killUT) then --累计击杀N个指定敌人
				local killNum = LuaGetPlayerCountVal(k, v[1])
				finishNum = finishNum + killNum
			elseif (k == hVar.MEDAL_TYPE.buildTT) then --累计建造N个塔
				local buildNum = LuaGetPlayerCountVal(k, v[1])
				finishNum = finishNum + buildNum
			elseif (k == hVar.MEDAL_TYPE.td_wj_001) or (k == hVar.MEDAL_TYPE.td_wj_002) then --无尽关卡1/2战绩
				--取2张无尽地图的较大值
				local nowN1 = LuaGetPlayerGamecenter_Challenge(hVar.MEDAL_TYPE.td_wj_001)
				local nowN2 = LuaGetPlayerGamecenter_Challenge(hVar.MEDAL_TYPE.td_wj_002)
				local nowN = math.max(nowN1, nowN2)
				finishNum = finishNum + nowN
			elseif (k == hVar.MEDAL_TYPE.starCount) then --获得的星星
				finishNum = LuaGetPlayerStarCountVal()
			elseif (k == hVar.MEDAL_TYPE.rescueScientist) then --累计拯救科学家的数量
				finishNum = LuaGetTankScientistNum()
			elseif (k == hVar.MEDAL_TYPE.tankDeadthCount) then --累计战车死亡的数量
				finishNum = LuaGetTankDeadthNum()
			else --一般情况
				local num = LuaGetPlayerCountVal(k)
		...
		--已领奖的成就显示已领取的标记
		if (state == 2) then --2:已领奖
			--文字灰掉
			_frmNode.childUI["TaskNode" .. index].childUI["taskDesc"].handle.s:setColor(ccc3(192, 192, 192))
			
			--已领奖的图标
			_frmNode.childUI["TaskNode" .. index].childUI["btnTakedReward"] = hUI.image:new({
				parent = _frmNode.childUI["TaskNode" .. index].handle._n,
				model = "UI:FinishTag",
				x = TASK_WIDTH / 2 - 168,
				y = 8,
			...
			_frmNode.childUI["TaskNode" .. index].childUI["btnTakedReward"].handle._n:setRotation(15)
		end
	...
	on_spine_screen_event = function()
		...
		--重绘本界面
		hGlobal.UI.InitMyTaskAchievementFrm("reload") --测试
		--触发事件，显示任务界面
		hGlobal.event:event("LocalEvent_Phone_ShowMyAchievement", current_medalType, current_funcCallback)
	end
	...
	--函数：点击某个成领奖按钮处理的逻辑
	OnSelectAchievementTakeRewardButton = function(task_idx)
		local _frm = hGlobal.UI.PhoneAchievementFrm
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		--print("OnSelectAchievementTakeRewardButton", task_idx)
		
		local tRecordI = current_AchievementInfo[task_idx]
		local medalId = tRecordI.medalId
		...
		--发起请求，领取成就奖励
		SendCmdFunc["achievement_takereward"](medalId)
	end
	
	--函数：刷新任务界面的滚动
	refresh_dlcmapinfo_UI_achievement_loop = function()
		local _frm = hGlobal.UI.PhoneAchievementFrm
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
	...
	--函数：异步绘制任务条目的timer
	refresh_async_paint_achievement_loop = function()
		local _frm = hGlobal.UI.PhoneAchievementFrm
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		
		--如果待异步绘制表里有内容，逐一绘制
		if (#current_async_paint_list_achievement > 0) then
			local loopCount = ASYNC_PAINTNUM_ONCE_TASK
			if (loopCount > #current_async_paint_list_achievement) then
				loopCount = #current_async_paint_list_achievement
			end
			...
			for loop = 1, loopCount, 1 do
				--取第一项
				local pivot = 1
				local tRecordI = current_async_paint_list_achievement[pivot]
				...
				--再创建实体控件
				on_create_single_achievement_UI(index, tRecordI)
				
				table.remove(current_async_paint_list_achievement, pivot)
			end
		end
	end
	...
	getUpDownOffset_task = function()
		...
		--最后一个任务的数据
		local ActivityBtnN = _frmNode.childUI["TaskNode" .. #current_AchievementInfo]
		...
		if ActivityBtnN then
			...
			deltNa_ry = btnN_ry + 561 + 138 --最后一个活动距离下侧边界的距离
		end
	...
	__DynamicAddRes = function()
		...
		--加载资源
		--动态加载宝物背景图
		local texture = CCTextureCache:sharedTextureCache():textureForKey("data/image/misc/task/achievement_panel.png")
		if (not texture) then
			texture = CCTextureCache:sharedTextureCache():addImage("data/image/misc/task/achievement_panel.png")
			print("加载任务背景大图！")
		end
	...
	RefreshTaskAchievementFinishPage = function()
		...
		--检测是否有可以领取的任务
		--local bHaveFinishedTask = hApi.CheckDailyQuest() --日常任务检测（返回 true/false, 有任务完成返回true）
		--_frm.childUI["PageBtn1"].childUI["NoteJianTou"].handle.s:setVisible(bHaveFinishedTask)
...
--监听关闭宝箱界面事件
hGlobal.event:listen("clearPhoneChestFrm", "__CloseAchievementFrm", function()
	if hGlobal.UI.PhoneAchievementFrm then
		if (hGlobal.UI.PhoneAchievementFrm.data.show == 1) then
			--关闭本界面
			OnClosePanel()
		end
...
--监听打开任务、成就、活动界面通知事件
hGlobal.event:listen("LocalEvent_Phone_ShowMyAchievement", "__ShowAchievementFrm", function(medalType, callback, bOpenImmediate)
	hGlobal.UI.InitMyTaskAchievementFrm("reload")
	...
	if bOpenImmediate then
		--显示道具界面
		hGlobal.UI.PhoneAchievementFrm:show(1)
		hGlobal.UI.PhoneAchievementFrm:active()
...
--[[
--测试代码
if hGlobal.UI.PhoneAchievementFrm then --任务成就面板
	hGlobal.UI.PhoneAchievementFrm:del()
	hGlobal.UI.PhoneAchievementFrm = nil
end
hGlobal.UI.InitMyTaskAchievementFrm("include") --测试
--触发事件，显示成就界面
local medalType = hVar.MEDAL_TYPE.rescueScientist
hGlobal.event:event("LocalEvent_Phone_ShowMyAchievement", medalType)
]]
