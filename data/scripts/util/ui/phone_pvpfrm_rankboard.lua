...
--竞技场排行榜操作面板
hGlobal.UI.InitRankBoradFrm_PVP = function(mode)
	--不重复创建
	if hGlobal.UI.PhoneRankBoardFrm_PVP then --排行榜操作面板
		return
	end
	...
	--分页1：排行榜函数部分
	local OnCreateRankBoardFrame_PVP = hApi.DoNothing --创建排行榜界面（第1个分页）
	local OnRefreshRankBoardFrame_PVP = hApi.DoNothing --显示某个指定的排行榜界面
	local OnCreateSingleBoardData_PVP = hApi.DoNothing --绘制单条排行榜的信息
	local OnCreateRankBoardTipFrame_PVP = hApi.DoNothing --夺塔奇兵排行榜规则介绍tip
	local refresh_rank_board_UI_loop_PVP = hApi.DoNothing --刷新排行榜界面的滚动
	local on_receive_refresh_systime_event_PVP = hApi.DoNothing --收到获得系统时间的回调
	local on_receive_billboardT_event_PVP = hApi.DoNothing --收到排行榜静态数据模板的回调
	local on_receive_billboard_event_PVP = hApi.DoNothing --收到排行榜当前排行数据的回调
	local on_enter_background_event_PVP = hApi.DoNothing --收到程序进入后台的回调
	local on_query_billboard_info_timer_PVP = hApi.DoNothing --定时向服务器查询当前的排行数据
	...
	hApi.clearTimer("__RANK_BOARD_UPDATE_PVP__")
	hApi.clearTimer("__RANK_QUERY_TIMER_PVP__")
	
	--创建排行榜操作面板
	hGlobal.UI.PhoneRankBoardFrm_PVP = hUI.frame:new(
	{
		x = BOARD_POS_X,
		y = BOARD_POS_Y,
		z = 10000,
		...
		codeOnDragEx = function(touchX, touchY, touchMode)
			...
			if (touchMode == 2) then --抬起事件
				...
				if _frm then
					--是否点击了夺塔奇兵按钮
					local btn = _frm.childUI["PageBtn1"]
					if btn then
	...
	local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
	local _parent = _frm.handle._n
	
	--左侧裁剪区域
	local _BTC_PageClippingRect_Rankboard = {0, -45 - 28 + BILLBOARD_NODE_OFFSET_Y, 900, 434, 0}
	local _BTC_pClipNode_Rankboard = hApi.CreateClippingNode(_frm, _BTC_PageClippingRect_Rankboard, 99, _BTC_PageClippingRect_Rankboard[5], "_BTC_pClipNode_Rankboard")
	...
	_frm.childUI["closeBtn"] = hUI.button:new({
		...
		code = function()
			...
			if _frm then
				...
				if _frmNode then
					--子按钮i
					local sbtn2 = _frmNode.childUI["SubPageBtn2"]
					if sbtn2 then --模拟点击后面的子按钮
						sbtn2.data.clear() --先清除数据
						sbtn2.data.code(sbtn2)
					end
				end
			end
		end,
	...
	OnClickPageBtn = function(pageIndex)
		...
		if (pageIndex == 1) then --分页1：排行榜
			--创建排行榜分页
			OnCreateRankBoardFrame_PVP(pageIndex)
		end
	...
	OnClosePanel = function()
		...
		--不显示排行榜面板
		hGlobal.UI.PhoneRankBoardFrm_PVP:show(0)
	...
	--函数：创建排行榜界面（第1个分页）
	OnCreateRankBoardFrame_PVP = function(pageIdx)
		local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
		local _frmNode = _frm.childUI["PageNode"]
		...
		_frmNode.childUI["BillboardPageDown"] = hUI.image:new({
			...
			y = BILLBOARD_NODE_OFFSET_Y - 559 + 36,
			scale = 1.0,
			z = 99999, --最前端显示
		})
		...
		_frmNode.childUI["BillboardPageDown_Btn"] = hUI.button:new({
			...
			dragbox = _frm.childUI["dragBox"],
			x = BILLBOARD_NODE_OFFSET_X + 328,
			y = BILLBOARD_NODE_OFFSET_Y - 566 + 33,
			w = 300,
			h = 50,
			scaleT = 1.0,
			z = 100000, --最前端显示
		...
		_frmNode.childUI["BillboardDragPanel"] = hUI.button:new({
			...
			y = BILLBOARD_NODE_OFFSET_Y - 307 + 18,
			w = 840,
			h = BILLBOARD_NODE_HEIGHT - 111,
			failcall = 1,
			...
			code = function(self, touchX, touchY, sus)
				...
				if (selected_billboardEx_idx > 0) then
					OnRefreshRankBoardFrame_PVP(selected_billboardEx_idx)
		...
		_frmNode.childUI["RankCol1"] = hUI.label:new({
			parent = _parentNode,
			x = BILLBOARD_NODE_OFFSET_X + 25 - 20,
		...
		_frmNode.childUI["RankCol2"] = hUI.label:new({
			parent = _parentNode,
			x = BILLBOARD_NODE_OFFSET_X + 25 + 180,
			y = BILLBOARD_NODE_OFFSET_Y - 140 + 87 - 1, --数字有1像素偏差
			size = 26,
			font = hVar.FONTC,
			align = "MC",
		...
		_frmNode.childUI["RankCol3"] = hUI.label:new({
			parent = _parentNode,
			x = BILLBOARD_NODE_OFFSET_X + 25 + 390,
			y = BILLBOARD_NODE_OFFSET_Y - 140 + 87 - 1, --数字有1像素偏差
			size = 26,
			font = hVar.FONTC,
			align = "MC",
			...
			border = 1,
			--text = "天梯分", --language
			text = hVar.tab_string["PVP_Point"], --language
		})
		...
		_frmNode.childUI["RankCol4"] = hUI.label:new({
			parent = _parentNode,
			x = BILLBOARD_NODE_OFFSET_X + 25 + 580,
			y = BILLBOARD_NODE_OFFSET_Y - 140 + 87 - 1, --数字有1像素偏差
			size = 26,
			font = hVar.FONTC,
			align = "MC",
		...
		_frmNode.childUI["MyRankColBG"] = hUI.image:new({
			...
			y = BILLBOARD_NODE_OFFSET_Y - 140 - 420 + 12,
			model = "UI:AttrBg", --UI:login_lk", --"misc/y_mask_16.png",
			w = 830,
			h = BILLBOARD_HEIGHT + 6,
		})
		...
		_frmNode.childUI["MyRankCol1"] = hUI.label:new({
			parent = _parentNode,
			x = BILLBOARD_NODE_OFFSET_X + 25 - 40 + 60 + 104,
		...
		_frmNode.childUI["MyRankValBG"] = hUI.image:new({
			parent = _parentNode,
			x = BILLBOARD_NODE_OFFSET_X + 25 - 40 + 220,
		...
		--排行榜规则(银杯)按钮（响应区域）
		_frmNode.childUI["btnBoardIntro_Silver"] = hUI.button:new({
			parent = _parentNode,
			dragbox = _frm.childUI["dragBox"],
			x = BILLBOARD_NODE_OFFSET_X - 40 - 5,
			y = BILLBOARD_NODE_OFFSET_Y - 140 - 400 + 5,
			w = 64,
			h = 64,
			model = "misc/mask.png",
			scaleT = 1.0,
			...
			code = function()
				--创建夺塔奇兵排行榜规则介绍tip(银杯)
				OnCreateRankBoardTipFrame_PVP("silver")
			end,
		})
		_frmNode.childUI["btnBoardIntro_Silver"].handle.s:setOpacity(0) --用于响应事件，不显示
		leftRemoveFrmList[#leftRemoveFrmList + 1] = "btnBoardIntro_Silver"
		--银杯图标
		_frmNode.childUI["btnBoardIntro_Silver"].childUI["icon"] = hUI.image:new({
			parent = _frmNode.childUI["btnBoardIntro_Silver"].handle._n,
			dragbox = _frm.childUI["dragBox"],
			x = 5,
			y = 0,
			scale = 0.9,
			model = "ui/pvp/quest_reward_m.png",
		})
		local towAction = CCSequence:createWithTwoActions(CCScaleTo:create(1.0, 0.92) ,CCScaleTo:create(1.0, 0.88))
		local forever = CCRepeatForever:create(tolua.cast(towAction,"CCActionInterval"))
		_frmNode.childUI["btnBoardIntro_Silver"].childUI["icon"].handle._n:runAction(forever)
		_frmNode.childUI["btnBoardIntro_Silver"].childUI["icon"].handle.s:setVisible(false) --一开始不显示银杯
		
		--排行榜规则(金杯)按钮（响应区域）
		_frmNode.childUI["btnBoardIntro_Gold"] = hUI.button:new({
			parent = _parentNode,
			dragbox = _frm.childUI["dragBox"],
			x = BILLBOARD_NODE_OFFSET_X - 40 + 60,
			y = BILLBOARD_NODE_OFFSET_Y - 140 - 400 + 5,
			w = 64,
			h = 64,
			model = "misc/mask.png",
			...
			code = function()
				--创建夺塔奇兵排行榜规则介绍tip(金杯)
				OnCreateRankBoardTipFrame_PVP("gold")
			end,
		})
		_frmNode.childUI["btnBoardIntro_Gold"].handle.s:setOpacity(0) --用于响应事件，不显示
		leftRemoveFrmList[#leftRemoveFrmList + 1] = "btnBoardIntro_Gold"
		--金杯图标
		_frmNode.childUI["btnBoardIntro_Gold"].childUI["icon"] = hUI.image:new({
			parent = _frmNode.childUI["btnBoardIntro_Gold"].handle._n,
			dragbox = _frm.childUI["dragBox"],
			x = -5,
			y = 0,
			scale = 0.9,
			model = "ui/pvp/quest_reward.png",
		})
		local towAction = CCSequence:createWithTwoActions(CCScaleTo:create(1.0, 0.92) ,CCScaleTo:create(1.0, 0.88))
		local forever = CCRepeatForever:create(tolua.cast(towAction,"CCActionInterval"))
		_frmNode.childUI["btnBoardIntro_Gold"].childUI["icon"].handle._n:runAction(forever)
		_frmNode.childUI["btnBoardIntro_Gold"].childUI["icon"].handle.s:setVisible(false) --一开始不显示金杯
		
		--添加本分页的监听事件
		--监听：收到服务器时间的回调
		hGlobal.event:listen("localEvent_refresh_Systime", "__QueryBillBoardTime", on_receive_refresh_systime_event_PVP)
		--监听：收到排行榜静态数据模板的回调
		hGlobal.event:listen("localEvent_refresh_billboardT", "__QueryBillBoardT", on_receive_billboardT_event_PVP)
		--监听：收到排行榜当前排行数据的回调
		hGlobal.event:listen("localEvent_refresh_billboard", "__QueryBillBoard", on_receive_billboard_event_PVP)
		--监听：程序进入后台的回调
		hGlobal.event:listen("LocalEvent_AppEnterBackground", "__QueryBillBoardEnterBG", on_enter_background_event_PVP)
		
		--创建只有本分页下才有的timer，刷新排行榜滚动
		hApi.addTimerForever("__RANK_BOARD_UPDATE_PVP__", hVar.TIMER_MODE.GAMETIME, 30, refresh_rank_board_UI_loop_PVP)
		
		--添加本分页的timer，定时取排行榜最新排名
		hApi.addTimerForever("__RANK_QUERY_TIMER_PVP__", hVar.TIMER_MODE.GAMETIME, QUERY_DELTA_SECOND * 1000, on_query_billboard_info_timer_PVP)
	...
	--函数：收到系统时间的回调
	on_receive_refresh_systime_event_PVP = function()
		local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		...
		if (current_STATE == 1) then --当前的状态(0:初始化 / 1:正在查询服务器时间 / 2:正在查询静态数据 / 3:查询完毕)
			...
			if (not current_billboardT) then
				...
				--本地已经存在排行榜静态数据模板
				on_receive_billboardT_event_PVP(current_billboardT)
			end
		end
	end
	
	--函数：收到排行榜静态数据模板的回调事件
	on_receive_billboardT_event_PVP = function(billboardT)
		local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		...
		if (current_STATE == 2) then --当前的状态(0:初始化 / 1:正在查询服务器时间 / 2:正在查询静态数据 / 3:查询完毕)
			...
			--显示金杯、银杯
			_frmNode.childUI["btnBoardIntro_Gold"].childUI["icon"].handle.s:setVisible(true)
			_frmNode.childUI["btnBoardIntro_Silver"].childUI["icon"].handle.s:setVisible(true)
			...
			local bId = current_billboardT.info[current_billId].bId
			...
			if (score > last_local_board_score) then
				...
				--print("存储最新的得分 update_billboard_rank", bId, score)
				
				--发起更新（会收到排行榜数据的异步回调）
				local strTime = os.date("%Y-%m-%d %H:%M:%S", timestamp)
				local checkSum = (score + timestamp) % 9973 --校验值
				...
				if (deltatime >= QUERY_DELTA_SECOND) then
					...
					--print("用本地存储的数据绘制界面")
					--用本地存储的数据绘制界面
					on_receive_billboard_event_PVP(current_billboardData)
				end
			--[[
			end
			]]
	...
	--函数：收到排行榜当前排行数据的回调事件
	on_receive_billboard_event_PVP = function(billboard)
		local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		...
		for i = 1, num, 1 do
			...
			--绘制该行
			OnCreateSingleBoardData_PVP(_BTC_pClipNode_Rankboard, i, offsetX, offsetY, info[i].name, info[i].rank, i, billboard)
		...
		--OnCreateSingleBoardData_PVP(_frmNode.handle._n, iMe, offsetXMe, offsetYMe, nameMe, rankMe, rankingMe, billboard)
		...
		elseif (rankingMe >= 4) and (rankingMe <= 10) then --第4-10名
			--蓝色
		...
		elseif (rankingMe == 3) then --第3名
			...
			_frmNode.childUI["BillboardNode" .. iMe].childUI["RankLabel"] = hUI.label:new({
				...
				width = 300,
				--border = 1,
				text = ((rankingMe == 0) and (current_billboardT.info[current_billId].prize.max_rank .. "+") or rankingMe),
			})
			_frmNode.childUI["BillboardNode" .. iMe].childUI["RankLabel"].handle.s:setColor(paintColor)
		end
		
		--"我的得分"值
		_frmNode.childUI["BillboardNode" .. iMe].childUI["RankScore"] = hUI.label:new({
			parent = _frmNode.childUI["BillboardNode" .. iMe].handle._n,
			x = 463 + 60 + 46,
			y = -3 + 11, --字体有1像素偏差
			size = 22,
		...
		_frmNode.childUI["BillboardNode" .. iMe].childUI["RankScore"].handle.s:setColor(paintColor)
	...
	--函数：收到程序进入后台的回调事件
	on_enter_background_event_PVP = function(flag)
	...
	--函数：绘制单条排行榜条目的内容
	OnCreateSingleBoardData_PVP = function(parent, index, offsetX, offsetY, name ,rank, ranking, billboard)
		local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		...
		--检测本排行应该获得何种奖励
		local giveRewardT = {}
		for r = 1, #current_billboardT.info[current_billId].prize.eom.reward, 1 do
			local rewardT = current_billboardT.info[current_billId].prize.eom.reward[r]
	...
	--函数：创建夺塔奇兵排行榜规则介绍tip
	OnCreateRankBoardTipFrame_PVP = function(strType)
		local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
		
		--先清除上一次的竞技场排行榜说明面板
		if hGlobal.UI.RankBoardInfoFrame_PVP then
			hGlobal.UI.RankBoardInfoFrame_PVP:del()
			hGlobal.UI.RankBoardInfoFrame_PVP = nil
		end
		...
		--创建排行榜说明面板
		hGlobal.UI.RankBoardInfoFrame_PVP = hUI.frame:new({
			x = 0,
			y = 0,
			w = hVar.SCREEN.w,
			h = hVar.SCREEN.h,
		...
		hGlobal.UI.RankBoardInfoFrame_PVP:active()
		
		local _RankTipParent = hGlobal.UI.RankBoardInfoFrame_PVP.handle._n
		local _RankTipChildUI = hGlobal.UI.RankBoardInfoFrame_PVP.childUI
		...
		local img9 = hApi.CCScale9SpriteCreateWithSpriteFrameName("data/image/ui/TacticBG.png", _offX, _offY - 235, 680, 560, hGlobal.UI.RankBoardInfoFrame_PVP)
		img9:setOpacity(204)
		
		--创建每日排行介绍tip-标题
		local strTitle = ""
		local giveRewardT = nil
		local paintColor = nil
		if (strType == "silver") then --银杯
			--strTitle = "竞技场赛季月中奖励" --language
			strTitle = hVar.tab_string["PVPSesonRewardI"] --language
			paintColor = ccc3(212, 212, 212) --银色
		elseif (strType == "gold") then --金杯
			--strTitle = "竞技场赛季月末奖励" --language
			strTitle = hVar.tab_string["PVPSesonRewardII"] --language
			paintColor = ccc3(255, 224, 0) --金色
		end
		...
		--创建每日排行介绍tip-内容1
		--local strHint1 = "夺塔奇兵竞技房进行有效局的战斗将获得天梯分数。每个月的月中和月末按照累计天梯分数的排名发放奖励。" --language
		local strHint1 = hVar.tab_string["PVPSesonRewardIntroI1"] --language
		...
		local strHint2 = nil
		if (strType == "silver") then --银杯
			--strHint2 = "月中奖励为16号结算发放。" --language
			strHint2 = hVar.tab_string["PVPSesonRewardIntroI2"] --language
		elseif (strType == "gold") then --金杯
			--strHint2 = "月末奖励为次月1号结算发放，并重新开始排行。" --language
			strHint2 = hVar.tab_string["PVPSesonRewardIntroII2"] --language
		end
		...
		local strHint3 = nil
		if (strType == "silver") then --银杯
			--strHint3 = "月中排名奖励：" --language
			strHint3 = hVar.tab_string["PVPSesonI"] .. hVar.tab_string["__TEXT_RankBoardHint3"] --language
		elseif (strType == "gold") then --金杯
			--strHint3 = "月末排名奖励：" --language
			strHint3 = hVar.tab_string["PVPSesonII"] .. hVar.tab_string["__TEXT_RankBoardHint3"] --language
		end
		...
		--各名次的奖励
		local AWARD_OFFSETY = 52
		local reward = current_billboardT.info[current_billId].prize.eom.reward
		for r = 1, #reward, 1 do
			local giveRewardT = nil
			if (strType == "silver") then
				giveRewardT = current_billboardT.info[current_billId].prize.mom.reward[r] --银杯
			elseif (strType == "gold") then
				giveRewardT = current_billboardT.info[current_billId].prize.eom.reward[r] --金杯
			end
			...
			_RankTipChildUI["RankBoardContent-RankRange"] = hUI.label:new({
				...
				size = 26,
				x = _offX - 295,
				y = _offY - 150 - 50 - (r - 1) * 64 + AWARD_OFFSETY,
				width = 650,
				align = "LT",
				font = hVar.FONTC,
				text = strRankRange,
			...
			for n = 1, #giveRewardT, 1 do
				...
				_RankTipChildUI["RankBoardContent-Reward".. n] = hUI.button:new({ --作为button是为了子控件坐标正常
					...
					y = _offY - 150 - 50 - (r - 1) * 64 - 9 + AWARD_OFFSETY,
					w = itemWidth * scaleModel,
					h = itemHeight * scaleModel,
				})
				...
				if sub_tmpModel then
					if (rewardT[1] == 5) then
						...
						_RankTipChildUI["RankBoardContent-Reward".. n].childUI["subIcon"] = hUI.image:new({
							...
							model = sub_tmpModel,
							x = sub_pos_x * scaleModel + 3,
							y = sub_pos_y * scaleModel - 6,
							w = sub_pos_w * scaleModel * 0.6,
							h = sub_pos_h * scaleModel * 0.6,
						})
					else
						_RankTipChildUI["RankBoardContent-Reward".. n].childUI["subIcon"] = hUI.image:new({
							parent = _RankTipChildUI["RankBoardContent-Reward".. n].handle._n,
				...
				_RankTipChildUI["RankBoardContent-RewardNum".. n] = hUI.label:new({
					parent = _RankTipParent,
					x = _offX - 100 + (n - 1) * WIDTH + 10,
					y = _offY - 150 - 50 - (r - 1) * 64 - 9 - 5 + AWARD_OFFSETY,
					size = scaleSize,
					font = "numWhite",
					align = "LC",
					width = 300,
			...
			if (#giveRewardT == 0) then
				...
				_RankTipChildUI["RankBoardContent-RankRange"].handle.s:setColor(ccc3(168, 168, 168))
			end
		end
	end
	
	--函数：定时向服务器查询当前的排行数据(timer)
	on_query_billboard_info_timer_PVP = function()
		--print("定时向服务器查询当前的排行数据(timer) on_query_billboard_info_timer_PVP")
		local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
				...
				if (g_cur_net_state == 1) then --联网状态
					if current_billboardT then --已获得排行榜的静态数据
						if (current_STATE == 3) then --当前的状态(0:初始化 / 1:正在查询服务器时间 / 2:正在查询静态数据 / 3:查询完毕)
							...
							local bId = current_billboardT.info[current_billId].bId
							--print("bId=", bId)
							--print("查询排行榜数据 on_query_billboard_info_timer_PVP get_billboard")
							SendCmdFunc["get_billboard"](bId)
						end
					end
				end
	...
	--函数：刷新排行榜界面的滚动
	refresh_rank_board_UI_loop_PVP = function()
		--如果当前在动画中，不用处理
		...
		if b_need_auto_fixing_billboard then
			...
			delta1_ly = btn1_ly + 78 --第一个排行榜距离上侧边界的距离
			...
			deltNa_ry = btnN_ry + 505 --最后一个排行榜距离下侧边界的距离
	...
	--函数：显示某个指定的排行榜界面
	OnRefreshRankBoardFrame_PVP = function(achieve_idx)
		local _frm = hGlobal.UI.PhoneRankBoardFrm_PVP
		local _frmNode = _frm.childUI["PageNode"]
		local _parentNode = _frmNode.handle._n
	...
	--监听打开竞技场排行榜界面通知事件
	hGlobal.event:listen("LocalEvent_Phone_ShowRankBorad_PVP", "__ShowRankBoardFrm_PVP", function(mapName)
		--触发事件，显示积分、金币界面
		--hGlobal.event:event("LocalEvent_ShowGameCoinFrm")
		
		--显示排行榜界面
		hGlobal.UI.PhoneRankBoardFrm_PVP:show(1)
		hGlobal.UI.PhoneRankBoardFrm_PVP:active()
...
--[[
--测试代码
if hGlobal.UI.PhoneRankBoardFrm_PVP then --删除上一次的
	hGlobal.UI.PhoneRankBoardFrm_PVP:del()
	hGlobal.UI.PhoneRankBoardFrm_PVP = nil
end
hGlobal.UI.InitRankBoradFrm_PVP() --测试
