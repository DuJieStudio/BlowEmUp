...
local COPY_PATH_BG = "data/image/maze/userdef/space_back"
local COPY_PATH_MAP = "data/image/maze/userdef/mazeskin"
local COPY_PATH_BG_SMALL = "data/image/maze/userdef/space_back_small"
local COPY_PATH_MAP_SMALL = "data/image/maze/userdef/mazeskin_small"
local COPY_PATH_BG_SMALL_SHORT = "maze/userdef/space_back_small"
local COPY_PATH_MAP_SMALL_SHORT = "maze/userdef/mazeskin_small"
local current_bg_postfix = "" --后缀名
local current_map_postfix = "" --后缀名
	

--创建GM领取资源界面（竖屏）
hGlobal.UI.InitPlayerGMResourceFrm_Vert = function(mode)
	local tInitEventName = {"LocalEvent_Phone_ShowGMResourceFrm_Vert", "GMResourceFrm_Vert"}
	if (mode ~= "include") then
		return tInitEventName
	end
	
	local _x,_y,_w,_h = hVar.SCREEN.w/2 - 700/2, hVar.SCREEN.h/2 + 1280/2 - 0, 700, 1280
	...
	local _cur_scroll_type = 0 --卷轴速度类型
	...
	local draggle_speed_y_redequip = {} --神器当前滑动的速度x
	...
	draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = 0 --神器当前滑动的速度x(武器)
	...
	draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = 0 --神器当前滑动的速度x(防具)
	...
	draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] = 0 --神器当前滑动的速度x(宝物)
	...
	draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] = 0 --神器当前滑动的速度x(坐骑)
	...
	local on_receive_album_selected_event = hApi.DoNothing --收到相册图片回调
	...
	local redWeapon_Origin = {"maze/space_back_01_small.png", "maze/space_back_02_small.png","maze/space_back_03_small.png","maze/space_back_04_small.png","maze/space_back_05_small.png","maze/space_back_06_small.png",}
	local redBody_Origin = {"maze/mazeskin_001_small.png", "maze/mazeskin_002_small.png","maze/mazeskin_003_small.png","maze/mazeskin_004_small.png","maze/mazeskin_005_small.png","maze/mazeskin_006_small.png","maze/mazeskin_011_small.png","maze/mazeskin_012_small.png","maze/mazeskin_013_small.png","maze/mazeskin_014_small.png","maze/mazeskin_015_small.png","maze/mazeskin_016_small.png","maze/mazeskin_017_small.png",}
	
	--当前相册操作索引
	local current_album_selectidx = 0
	...
	--删除上一次的领取神器界面
	if hGlobal.UI.PlayerGMResourceFrm_Vert then
		hGlobal.UI.PlayerGMResourceFrm_Vert:del()
		hGlobal.UI.PlayerGMResourceFrm_Vert = nil
	end
	...
	--移除事件监听：相册读取图片
	hGlobal.event:listen("LocalEvent_OnAlbumSelected", "OnAlbumSelected", nil)
	
	--领取神器界面
	hGlobal.UI.PlayerGMResourceFrm_Vert = hUI.frame:new({
		x = _x,
		y = _y,
		dragable = 2,
		w = _w,
	...
	local _frm = hGlobal.UI.PlayerGMResourceFrm_Vert
	...
	--左侧裁剪区域-红装神器
	local _BTC_PageClippingRect_RedEquip = {20, -160, 670, 700, 0,}
	...
	_childUI["closeBtn"] = hUI.button:new({
		...
		model = "misc/tempbag/close.png", --"BTN:PANEL_CLOSE",
		x = _frm.data.w - 34 - 20,
		y = 0,
		scaleT = 0.95,
		code = function()
	...
	on_receive_systime_gmresource = function()
		...
		--local redEquipIdList = {}
		local redWeapon = {}
		for k = 1, #redWeapon_Origin, 1 do
			redWeapon[#redWeapon+1] = redWeapon_Origin[k]
		end
		local redBody = {}
		for k = 1, #redBody_Origin, 1 do
			redBody[#redBody+1] = redBody_Origin[k]
		end
		local redOrnamesnts = {}
		local redHorse = {}
		...
		--[[
		local g_localfilepath_top = string.sub(g_localfilepath, 1, #g_localfilepath-11)
		local checkfilePath = g_localfilepath_top .. COPY_PATH_BG .. ".jpg"
		local ret1 = hApi.FileExists(checkfilePath,"full")
		--弹框
		local strText = "检测文件文件存在? path=["..tostring(checkfilePath) .. "]".."ret=" .. tostring(ret1)
		hGlobal.UI.MsgBox(strText, {
			font = hVar.FONTC,
			ok = function()
			end,
		})
		local imgFileName = COPY_PATH_BG_SMALL_SHORT .. ".jpg"
		local texture = CCTextureCache:sharedTextureCache():textureForKey(imgFileName)
		--弹框
		local strText = "加载贴图? path=["..tostring(imgFileName) .. "]".."ret=" .. tostring(texture)
		hGlobal.UI.MsgBox(strText, {
			font = hVar.FONTC,
			ok = function()
			end,
	...
	--函数：收到相册图片回调
	on_receive_album_selected_event = function(t)
		--只读第一个
		if (type(t) == "table") then
			if (#t > 0) then
				local tImgInfo = t[1]
				local imgFullPath = tImgInfo[1]
				
				--[[
				local strText = imgFullPath
				hUI.floatNumber:new({
					x = hVar.SCREEN.w / 2,
					y = hVar.SCREEN.h / 2,
					align = "MC",
				...
				--拷贝缩略图
				local g_localfilepath_top = string.sub(g_localfilepath, 1, #g_localfilepath-11)
				local filePostfix = string.sub(imgFullPath, string.find(imgFullPath, "%."), #imgFullPath) --后缀名
				filePostfix = string.lower(filePostfix) --后缀名转小写
				local copyPath = ""
				local copyPath_small = ""
				if (current_album_selectidx == 1) then --操作背景图
					copyPath = g_localfilepath_top .. COPY_PATH_BG .. filePostfix
					copyPath_small = g_localfilepath_top .. COPY_PATH_BG_SMALL .. filePostfix
					
					current_bg_postfix = filePostfix --后缀名
				elseif (current_album_selectidx == 2) then --操作地图
					copyPath = g_localfilepath_top .. COPY_PATH_MAP .. filePostfix
					copyPath_small = g_localfilepath_top .. COPY_PATH_MAP_SMALL .. filePostfix
					
					current_map_postfix = filePostfix --后缀名
				end
				if xlUtil_FileCopy then
					xlUtil_FileCopy(imgFullPath, copyPath)
					--print("copyPath OK!")
					--xlLG("diablo", "copyPath OK!\n")
				end
				if xlUtil_FileCopy then
				--if xlUtil_ThumbImg then
					--xlUtil_ThumbImg(imgFullPath, copyPath_small, 128, 128)
					xlUtil_FileCopy(imgFullPath, copyPath_small)
					--print("copyPath_small OK!")
					--xlLG("diablo", "copyPath_small OK!\n")
					--xlLG("diablo", "检测文件是否存在[" .. tostring(copyPath_small).."]")
					if (not hApi.FileExists(copyPath_small,"full")) then
						--弹框
						local strText = "文件不存在！path=["..tostring(copyPath_small) .. "]"
						hGlobal.UI.MsgBox(strText, {
							font = hVar.FONTC,
							ok = function()
							end,
				...
				--加载此缩略图
				if (current_album_selectidx == 1) then --操作背景图
					hApi.addTimerOnce("__TIMER_ONCE_USERDEF_ALBUM1__",1000,function()
						local redWeapon = {}
						for k = 1, #redWeapon_Origin, 1 do
							redWeapon[#redWeapon+1] = redWeapon_Origin[k]
						end
						redWeapon[#redWeapon+1] = COPY_PATH_BG_SMALL_SHORT .. filePostfix
						
						current_equiptypenum[hVar.ITEM_TYPE.WEAPON] = #redWeapon --武器
						_cur_index[hVar.ITEM_TYPE.WEAPON] = 0
						_tRedBagListOrigin[hVar.ITEM_TYPE.WEAPON] = redWeapon --红装原始列表
						--重绘
						_createRedGiftBagInfo(redWeapon, hVar.ITEM_TYPE.WEAPON)
						
						--选中自定义图片
						selected_redequip_idx[hVar.ITEM_TYPE.WEAPON] = #redWeapon
						OnClickedEquipBtn(hVar.ITEM_TYPE.WEAPON, selected_redequip_idx[hVar.ITEM_TYPE.WEAPON])
						
						--拉到底部
						b_need_auto_fixing_redequip[hVar.ITEM_TYPE.WEAPON] = true
						draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = 10000
					end)
				elseif (current_album_selectidx == 2) then --操作地图
					hApi.addTimerOnce("__TIMER_ONCE_USERDEF_ALBUM2__",1000,function()
						local redBody = {}
						for k = 1, #redBody_Origin, 1 do
							redBody[#redBody+1] = redBody_Origin[k]
						end
						redBody[#redBody+1] = COPY_PATH_MAP_SMALL_SHORT .. filePostfix
						
						current_equiptypenum[hVar.ITEM_TYPE.BODY] = #redBody --武器
						_cur_index[hVar.ITEM_TYPE.BODY] = 0
						_tRedBagListOrigin[hVar.ITEM_TYPE.BODY] = redBody --红装原始列表
						--重绘
						_createRedGiftBagInfo(redBody, hVar.ITEM_TYPE.BODY)
						
						--选中自定义图片
						selected_redequip_idx[hVar.ITEM_TYPE.BODY] = #redBody
						OnClickedEquipBtn(hVar.ITEM_TYPE.BODY, selected_redequip_idx[hVar.ITEM_TYPE.BODY])
						
						--拉到底部
						b_need_auto_fixing_redequip[hVar.ITEM_TYPE.BODY] = true
						draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = 10000
					end)
				end
	...
	_createRedGiftBagInfo = function(redEquipIdList, equipType)
		for i = 1, #redEquipIdList, 1 do
			...
			if imgName then
				...
				local itemPosY = 0
				local itemWH = 96
				local itemBorder = nil
				if (equipType == hVar.ITEM_TYPE.WEAPON) then --武器
					itemPosX = _itemX + 30
					itemPosY = _itemY - 130 - _itemOffY * 0 - (i - 1) * (_itemOffX + 110)
					itemWH = 240
					itemBorder = "misc/ugc/ugc_border1.png"
				elseif (equipType == hVar.ITEM_TYPE.BODY) then --防具
					itemPosX = _itemX + 360
					itemPosY = _itemY - 130 - _itemOffY * 0 - (i - 1) * (_itemOffX + 110)
					itemWH = 240
					itemBorder = "misc/ugc/ugc_border2.png"
				...
				hApi.safeRemoveT(_childUI, "RedEquip_" .. equipType .. "_" .. i) 
				...
				_childUI["RedEquip_" .. equipType .. "_" .. i].childUI["bg"] = hUI.image:new({
					...
					model = itemBorder, --"misc/mask_white.png",
					x = 0,
					y = 0,
					w = itemWH,
					h = itemWH,
				...
				--print("图标")
				--xlLG("diablo", "图标")
				_childUI["RedEquip_" .. equipType .. "_" .. i].childUI["icon"] = hUI.button:new({ --作为按钮只是为了挂载子控件
					parent = _childUI["RedEquip_" .. equipType .. "_" .. i].handle._n,
					--model = imgName,
					model = -1,
					x = 0,
				...
				local imgFileName = "data/image/" .. imgName
				--print("imgFileName=", imgFileName)
				--xlLG("diablo", "imgFileName=" .. tostring(imgFileName))
				local texture = CCTextureCache:sharedTextureCache():textureForKey(imgFileName)
				if (not texture) then
					texture = CCTextureCache:sharedTextureCache():addImage(imgFileName)
					print("加载" .. imgFileName .. "！")
					--xlLG("diablo", "加载" .. tostring(imgFileName))
				end
				--print("texture=", texture)
				--xlLG("diablo", "texture=" .. tostring(texture))
				local sprite = CCSprite:createWithTexture(texture)
				local size = sprite:getContentSize()
				sprite:setAnchorPoint(ccp(0.5, 0.5))
				sprite:setScaleX((itemWH - 12)/size.width)
				sprite:setScaleY((itemWH - 12)/size.height)
				sprite:setPosition(ccp(0, 0)) --修复在地图最上面看不到砖块图片了
				_childUI["RedEquip_" .. equipType .. "_" .. i].childUI["icon"].handle._n:addChild(sprite)
				--print(size.width, size.height)
				--[[
				--文字
				_childUI["RedEquip_" .. equipType .. "_" .. i].childUI["label"] = hUI.label:new({
					parent = _childUI["RedEquip_" .. equipType .. "_" .. i].handle._n,
					x = 0,
					y = 64,
					align = "MC",
				...
				local s9 = hApi.CCScale9SpriteCreate("data/image/misc/ugc/ugc_border_select.png", 0, 0, itemWH, itemWH, _childUI["RedEquip_" .. equipType .. "_" .. i].childUI["selectbox"])
				--s9:setOpacity(128)
	...
	_createRedGiftBagClickAreaInfo = function()
		...
		--分界线
		_childUI["apartline_1"] = hUI.button:new({ --作为按钮只是为了挂载子控件
			parent = _parent,
			model = -1,
			x = _w/2,
			y = -_h+60,
		...
		local s9 = hApi.CCScale9SpriteCreate("data/image/misc/ugc/ugc_background.png", 0, 0, _w+20, 400, _childUI["apartline_1"])
		s9:setOpacity(212)
		--local s9 = hApi.CCScale9SpriteCreate("data/image/misc/ugc/ugc_border_select.png", 0, 0, _w+20, 400, _childUI["apartline_1"])
		...
		--标题图片
		_childUI["title"] = hUI.image:new({
			parent = _parent,
			align = "MC",
			model = "misc/ugc/ugc_title.png",
			x = 260,
			y = 0,
			scale = 0.8,
		})
		...
		_RemoveList[#_RemoveList+1] = "tipLab4"
		...
		_childUI["RedEquipPageLeft_Weapon"].handle.s:setRotation(90)
		_childUI["RedEquipPageLeft_Weapon"].handle.s:setOpacity(1) --提示上翻页默认透明度为212
		...
		_childUI["RedEquipPageRight_Weapon"] = hUI.image:new({
			...
			model = "UI:PageBtn",
			x = _itemX + 25 + 4,--非对称式
			y = _itemY - 675-40, --非对称式
			scale = 1.0,
			z = 99999, --最前端显示
		})
		_childUI["RedEquipPageRight_Weapon"].handle.s:setRotation(270)
		_childUI["RedEquipPageRight_Weapon"].handle.s:setOpacity(1) --提示下翻页默认透明度为212
		...
		_childUI["RedEquipPageLeft_Btn_Weapon"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				...
				if (equiptypenum > PAGE_X_NUM[hVar.ITEM_TYPE.WEAPON]) then
					...
					draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.WEAPON] / 2.0 --左速度
				end
			end,
		})
		...
		_childUI["RedEquipPageRight_Btn_Weapon"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				...
				if (equiptypenum > PAGE_X_NUM[hVar.ITEM_TYPE.WEAPON]) then
					...
					b_need_auto_fixing_redequip[hVar.ITEM_TYPE.WEAPON] = true
					friction_redequip[hVar.ITEM_TYPE.WEAPON] = 0
					draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.WEAPON] / 2.0 --右速度
				end
			end,
		})
		_childUI["RedEquipPageRight_Btn_Weapon"].handle.s:setOpacity(1) --只用于控制，不显示
		...
		_childUI["RedEquipPageLeft_Body"].handle.s:setRotation(90)
		_childUI["RedEquipPageLeft_Body"].handle.s:setOpacity(0) --提示上翻页默认透明度为212
		...
		_childUI["RedEquipPageRight_Body"].handle.s:setRotation(270)
		_childUI["RedEquipPageRight_Body"].handle.s:setOpacity(1) --提示下翻页默认透明度为212
		--如果神器数量未铺满第一页，那么不显示下翻页提示
		--if (#hVar.RedEquip <= PAGE_X_NUM) then
		...
		_childUI["RedEquipPageLeft_Btn_Body"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				...
				if (equiptypenum > PAGE_X_NUM[hVar.ITEM_TYPE.BODY]) then
					...
					draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.BODY] / 2.0 --左速度
				end
			end,
		})
		...
		_childUI["RedEquipPageRight_Btn_Body"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				...
				if (equiptypenum > PAGE_X_NUM[hVar.ITEM_TYPE.BODY]) then
					...
					b_need_auto_fixing_redequip[hVar.ITEM_TYPE.BODY] = true
					friction_redequip[hVar.ITEM_TYPE.BODY] = 0
					draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.BODY] / 2.0 --右速度
				end
			end,
		})
		...
		_childUI["RedEquipPageLeft_Btn_Ornaments"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				...
				if (equiptypenum > PAGE_X_NUM[hVar.ITEM_TYPE.ORNAMENTS]) then
					...
					draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] = MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.ORNAMENTS] / 2.0 --左速度
				end
			end,
		})
		...
		_childUI["RedEquipPageRight_Btn_Ornaments"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				...
				if (equiptypenum > PAGE_X_NUM[hVar.ITEM_TYPE.ORNAMENTS]) then
					...
					b_need_auto_fixing_redequip[hVar.ITEM_TYPE.ORNAMENTS] = true
					friction_redequip[hVar.ITEM_TYPE.ORNAMENTS] = 0
					draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] = -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.ORNAMENTS] / 2.0 --右速度
				end
			end,
		})
		...
		_childUI["RedEquipPageLeft_Btn_Mount"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				...
				if (equiptypenum > PAGE_X_NUM[hVar.ITEM_TYPE.MOUNT]) then
					...
					draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] = MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.MOUNT] / 2.0 --左速度
				end
			end,
		})
		...
		_childUI["RedEquipPageRight_Btn_Mount"] = hUI.button:new({
			...
			code = function(self, screenX, screenY, isInside)
				...
				if (equiptypenum > PAGE_X_NUM[hVar.ITEM_TYPE.MOUNT]) then
					...
					b_need_auto_fixing_redequip[hVar.ITEM_TYPE.MOUNT] = true
					friction_redequip[hVar.ITEM_TYPE.MOUNT] = 0
					draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] = -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.MOUNT] / 2.0 --右速度
				end
			end,
		})
		...
		_childUI["RedEquipDragPanel_Weapon"] = hUI.button:new({
			...
			model = "misc/mask.png",
			dragbox = _frm.childUI["dragBox"],
			x = _itemX + (_itemOffX * PAGE_X_NUM[hVar.ITEM_TYPE.WEAPON]) / 2 - _itemOffX / 2 - 210,
			y = _itemY - 295-40,
			w = 320,
			h = 700,
			failcall = 1,
			...
			codeOnTouch = function(self, touchX, touchY, sus)
				...
				draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = 0 --当前速度为0
			...
			codeOnDrag = function(self, touchX, touchY, sus)
				...
				--处理移动速度y
				draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = touchY - last_click_pos_y_redequip[hVar.ITEM_TYPE.WEAPON]
				
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] > MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.WEAPON]) then
					draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.WEAPON]
				end
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] < -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.WEAPON]) then
					draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.WEAPON]
				end
				...
				if click_scroll_redequip[hVar.ITEM_TYPE.WEAPON] then
					local deltaY = touchY - last_click_pos_y_redequip[hVar.ITEM_TYPE.WEAPON] --与开始按下的位置的偏移值x
					
					--第一个DLC地图面板的坐标不能跑到最左侧的右边去
					if ((delta1_lx + deltaY) <= 0) then --防止走过
						deltaY = -delta1_lx
						delta1_lx = 0
						--没有惯性
						draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = 0
					...
					if ((deltNa_rx + deltaY) >= 0) then --防止走过
						deltaY = -deltNa_rx
						deltNa_rx = 0
			...
			code = function(self, touchX, touchY, sus)
				...
				if (click_scroll_redequip[hVar.ITEM_TYPE.WEAPON]) and (math.abs(touchY - click_pos_y_redequip[hVar.ITEM_TYPE.WEAPON]) > 48) then
		...
		_childUI["RedEquipDragPanel_Body"] = hUI.button:new({
			...
			model = "misc/mask.png",
			dragbox = _frm.childUI["dragBox"],
			x = _itemX + (_itemOffX * PAGE_X_NUM[hVar.ITEM_TYPE.WEAPON]) / 2 - _itemOffX / 2 + 120,
			y = _itemY - 295-40,
			w = 320,
			h = 700,
			failcall = 1,
			...
			codeOnTouch = function(self, touchX, touchY, sus)
				...
				draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = 0 --当前速度为0
			...
			codeOnDrag = function(self, touchX, touchY, sus)
				--print("codeOnDrag", touchX, touchY, sus)
				--处理移动速度y
				draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = touchY - last_click_pos_y_redequip[hVar.ITEM_TYPE.BODY]
				
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] > MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.BODY]) then
					draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.BODY]
				end
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] < -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.BODY]) then
					draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.BODY]
				end
				...
				if click_scroll_redequip[hVar.ITEM_TYPE.BODY] then
					local deltaY = touchY - last_click_pos_y_redequip[hVar.ITEM_TYPE.BODY] --与开始按下的位置的偏移值x
					...
					if ((delta1_lx + deltaY) <= 0) then --防止走过
						...
						--没有惯性
						draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = 0
			...
			code = function(self, touchX, touchY, sus)
				...
				--这种情况请注意：在触发滑动操作（英雄卡数量大于一页），并且按下坐标和抬起坐标偏差过大，则认为该操作是滑动，不触发选择卡牌
				if (click_scroll_redequip[hVar.ITEM_TYPE.BODY]) and (math.abs(touchY - click_pos_y_redequip[hVar.ITEM_TYPE.BODY]) > 48) then
		...
		_childUI["RedEquipDragPanel_Ornaments"] = hUI.button:new({
			...
			codeOnTouch = function(self, touchX, touchY, sus)
				...
				draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] = 0 --当前速度为0
			...
			codeOnDrag = function(self, touchX, touchY, sus)
				--print("codeOnDrag", touchX, touchY, sus)
				--处理移动速度y
				draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] = touchX - last_click_pos_x_redequip[hVar.ITEM_TYPE.ORNAMENTS]
				
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] > MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.ORNAMENTS]) then
					draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] = MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.ORNAMENTS]
				end
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] < -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.ORNAMENTS]) then
					draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] = -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.ORNAMENTS]
				end
				...
				if click_scroll_redequip[hVar.ITEM_TYPE.ORNAMENTS] then
					...
					if ((delta1_lx + deltaX) >= 0) then --防止走过
						...
						--没有惯性
						draggle_speed_y_redequip[hVar.ITEM_TYPE.ORNAMENTS] = 0
		...
		_childUI["RedEquipDragPanel_Mount"] = hUI.button:new({
			...
			codeOnTouch = function(self, touchX, touchY, sus)
				...
				draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] = 0 --当前速度为0
			...
			codeOnDrag = function(self, touchX, touchY, sus)
				--print("codeOnDrag", touchX, touchY, sus)
				--处理移动速度y
				draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] = touchX - last_click_pos_x_redequip[hVar.ITEM_TYPE.MOUNT]
				
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] > MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.MOUNT]) then
					draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] = MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.MOUNT]
				end
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] < -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.MOUNT]) then
					draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] = -MAX_SPEED_REDEQUIP[hVar.ITEM_TYPE.MOUNT]
				end
				...
				if click_scroll_redequip[hVar.ITEM_TYPE.MOUNT] then
					...
					if ((delta1_lx + deltaX) >= 0) then --防止走过
						...
						--没有惯性
						draggle_speed_y_redequip[hVar.ITEM_TYPE.MOUNT] = 0
		...
		--加号按钮1
		_childUI["DLCMapInfo_Btn_AddBG1"] = hUI.button:new({
			parent = _parent,
			model = "misc/ugc/ugc_addbtn.png", --"misc/addition/icon_add.png",
			dragbox = _frm.childUI["dragBox"],
			x = _itemX - 40 + 70,
			y = _itemY - 510 - 1 - 50*5-10,
			scale = 1,
			scaleT = 0.95,
			code = function()
				--标记操作相册选择背景图
				current_album_selectidx = 1
				
				if xlUtil_AlbumSelector then
					xlUtil_AlbumSelector()
				end
				
				if xlUtil_AlbumSelector == nil then
					--测试 --test
					local t = {{"C:\\Users\\mac\\Desktop\\space_back_07.jpg", 2560, 2560},}
					xlLuaEvent_OnAlbumSelected(t)
				end
			end,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_Btn_AddBG1"
		
		--加号按钮2
		_childUI["DLCMapInfo_Btn_AddBG2"] = hUI.button:new({
			parent = _parent,
			model = "misc/ugc/ugc_addbtn.png", --"misc/addition/icon_add.png",
			dragbox = _frm.childUI["dragBox"],
			x = _itemX - 40 + 400,
			...
			code = function()
				...
				if xlUtil_AlbumSelector == nil then
					--测试 --test
					local t = {{"C:\\Users\\mac\\Desktop\\mazeskin_007_small.png", 2560, 2560},}
					xlLuaEvent_OnAlbumSelected(t)
				end
			end,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_Btn_AddBG2"
		
		------------------------------------------------------------------------------
		local wallOffsetX = 100
		local wallOffsetY = -90
		--[[
		--墙体1背景
		_childUI["DLCMapInfo_SC1_SelectBG"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_border.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX,
			y = _itemY - 590 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_SC1_SelectBG"
		]]
		
		--墙体1选中框
		_childUI["DLCMapInfo_SC1_Selectbox"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_wall1.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX,
			y = _itemY - 590 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		...
		_childUI["DLCMapInf_SC1_Label"] = hUI.label:new({
			...
			x = _itemX - 40 + wallOffsetX,
			y = _itemY - 590 - 1 - 50*5 + 19+wallOffsetY,
			text = "高",
			width = 500,
			align = "LT",
			border = 1,
		...
		_childUI["DLCMapInfo_SC1_Selectbox_Finish"] = hUI.image:new({
			...
			y = _itemY - 590 - 1 - 50*5+wallOffsetY,
			scale = 1.0,
			model = "misc/ugc/ugc_border_select.png", --"UI:finish",
			w = 64,
			h = 64,
		})
		...
		_childUI["DLCMapInfo_SC1_Btn"] = hUI.button:new({
			...
			code = function(self)
				if (_cur_wall_type ~= 1) then
					...
					_childUI["DLCMapInfo_SC1_Selectbox_Finish"].handle._n:runAction(CCSequence:createWithTwoActions(CCScaleTo:create(0.05,0.52,0.52),CCScaleTo:create(0.05,0.5,0.5)))
					...
					if btnOk then
						...
						_childUI["confirmBtn"].childUI["image1"].handle._n:setVisible(true)
						_childUI["confirmBtn"].childUI["image2"].handle._n:setVisible(true)
					end
				else
					--隐藏全部选中框
					...
					_childUI["confirmBtn"].childUI["image1"].handle._n:setVisible(false)
					_childUI["confirmBtn"].childUI["image2"].handle._n:setVisible(false)
				end
			end,
		})
		...
		--[[
		--墙体2背景
		_childUI["DLCMapInfo_SC2_SelectBG"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_border.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*1,
			y = _itemY - 590 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_SC2_SelectBG"
		]]
		
		--墙体2选中框
		_childUI["DLCMapInfo_SC2_Selectbox"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_wall2.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*1,
			y = _itemY - 590 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		...
		_childUI["DLCMapInfo_SC2_Btn"] = hUI.button:new({
			...
			code = function(self)
				if (_cur_wall_type ~= 2) then
					...
					_childUI["DLCMapInfo_SC2_Selectbox_Finish"].handle._n:runAction(CCSequence:createWithTwoActions(CCScaleTo:create(0.05,0.52,0.52),CCScaleTo:create(0.05,0.5,0.5)))
		...
		--[[
		--墙体3背景
		_childUI["DLCMapInfo_SC3_SelectBG"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_border.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*2,
			y = _itemY - 590 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_SC3_SelectBG"
		]]
		
		--墙体3选中框
		_childUI["DLCMapInfo_SC3_Selectbox"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_wall3.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*2,
			y = _itemY - 590 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		...
		_childUI["DLCMapInfo_SC3_Btn"] = hUI.button:new({
			...
			code = function(self)
				if (_cur_wall_type ~= 3) then
					...
					_childUI["DLCMapInfo_SC3_Selectbox_Finish"].handle._n:runAction(CCSequence:createWithTwoActions(CCScaleTo:create(0.05,0.52,0.52),CCScaleTo:create(0.05,0.5,0.5)))
		...
		--[[
		--卷轴1背景
		_childUI["DLCMapInfo_JZ1_SelectBG"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_border.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*0,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ1_SelectBG"
		]]
		
		--卷轴速度1选中框
		_childUI["DLCMapInfo_JZ1_Selectbox"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_scroll1.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*0,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ1_Selectbox"
		...
		_childUI["DLCMapInf_SC1_Label"] = hUI.label:new({
			...
			model = "UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX,
			y = _itemY - 700 - 1 - 50*5 + 19+wallOffsetY,
			text = "快",
			width = 500,
			align = "LT",
			border = 1,
		...
		--卷轴速度1选中的特效
		_childUI["DLCMapInfo_JZ1_Selectbox_Finish"] = hUI.image:new({
			parent = _parent,
			x = _itemX - 40 + wallOffsetX,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
			model = "misc/ugc/ugc_border_select.png", --"UI:finish",
		...
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ1_Selectbox_Finish"
		--_childUI["DLCMapInfo_JZ1_Selectbox_Finish"].handle._n:setVisible(false) --默认隐藏选中框
		_cur_scroll_type = 1
		
		--卷轴速度1事件按钮
		_childUI["DLCMapInfo_JZ1_Btn"] = hUI.button:new({
			parent = _parent,
			dragbox = _frm.childUI["dragBox"],
			model = -1,
			--model = "misc/mask.png",
			...
			h = 64,
			code = function(self)
				if (_cur_scroll_type ~= 1) then
					--显示选中框1
					_childUI["DLCMapInfo_JZ1_Selectbox_Finish"].handle._n:setVisible(true)
					_childUI["DLCMapInfo_JZ1_Selectbox_Finish"].handle._n:runAction(CCSequence:createWithTwoActions(CCScaleTo:create(0.05,0.52,0.52),CCScaleTo:create(0.05,0.5,0.5)))
					
					--隐藏其他选中框
					_childUI["DLCMapInfo_JZ2_Selectbox_Finish"].handle._n:setVisible(false)
					_childUI["DLCMapInfo_JZ3_Selectbox_Finish"].handle._n:setVisible(false)
					...
					--隐藏全部选中框
					_childUI["DLCMapInfo_JZ1_Selectbox_Finish"].handle._n:setVisible(false)
					_childUI["DLCMapInfo_JZ2_Selectbox_Finish"].handle._n:setVisible(false)
					_childUI["DLCMapInfo_JZ3_Selectbox_Finish"].handle._n:setVisible(false)
		...
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ1_Btn"
		
		------------------------------------------------------------------------------
		--[[
		--卷轴2背景
		_childUI["DLCMapInfo_JZ2_SelectBG"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_border.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*1,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
		...
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ2_SelectBG"
		]]
		
		--卷轴速度2选中框
		_childUI["DLCMapInfo_JZ2_Selectbox"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_scroll2.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*1,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ2_Selectbox"
		...
		_childUI["DLCMapInf_SC2_Label"] = hUI.label:new({
			...
			model = "UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*1,
			y = _itemY - 680 - 1 - 50*5 + 19+wallOffsetY,
			text = "慢",
			width = 500,
			align = "LT",
			border = 1,
		...
		--卷轴速度2选中的特效
		_childUI["DLCMapInfo_JZ2_Selectbox_Finish"] = hUI.image:new({
			parent = _parent,
			x = _itemX - 40 + wallOffsetX + 170*1,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
			model = "misc/ugc/ugc_border_select.png", --"UI:finish",
		...
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ2_Selectbox_Finish"
		_childUI["DLCMapInfo_JZ2_Selectbox_Finish"].handle._n:setVisible(false) --默认隐藏选中框
		
		--卷轴速度2事件按钮
		_childUI["DLCMapInfo_JZ2_Btn"] = hUI.button:new({
			parent = _parent,
			dragbox = _frm.childUI["dragBox"],
			model = -1,
			--model = "misc/mask.png",
			...
			h = 64,
			code = function(self)
				if (_cur_scroll_type ~= 2) then
					--显示选中框2
					_childUI["DLCMapInfo_JZ2_Selectbox_Finish"].handle._n:setVisible(true)
					_childUI["DLCMapInfo_JZ2_Selectbox_Finish"].handle._n:runAction(CCSequence:createWithTwoActions(CCScaleTo:create(0.05,0.52,0.52),CCScaleTo:create(0.05,0.5,0.5)))
		...
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ2_Btn"
		
		------------------------------------------------------------------------------
		--[[
		--卷轴3背景
		_childUI["DLCMapInfo_JZ3_SelectBG"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_border.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*2,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
		...
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ3_SelectBG"
		]]
		
		--卷轴速度3选中框
		_childUI["DLCMapInfo_JZ3_Selectbox"] = hUI.image:new({
			parent = _parent,
			model = "misc/ugc/ugc_scroll3.png", --"UI:Button_SelectBorder",
			x = _itemX - 40 + wallOffsetX + 170*2,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
			w = 64,
			h = 64,
		})
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ3_Selectbox"
		...
		--卷轴速度3选中的特效
		_childUI["DLCMapInfo_JZ3_Selectbox_Finish"] = hUI.image:new({
			parent = _parent,
			x = _itemX - 40 + wallOffsetX + 170*2,
			y = _itemY - 700 - 1 - 50*5+wallOffsetY,
			model = "misc/ugc/ugc_border_select.png", --"UI:finish",
		...
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ3_Selectbox_Finish"
		_childUI["DLCMapInfo_JZ3_Selectbox_Finish"].handle._n:setVisible(false) --默认隐藏选中框
		
		--卷轴速度3事件按钮
		_childUI["DLCMapInfo_JZ3_Btn"] = hUI.button:new({
			parent = _parent,
			dragbox = _frm.childUI["dragBox"],
			model = -1,
			--model = "misc/mask.png",
			...
			h = 64,
			code = function(self)
				if (_cur_scroll_type ~= 3) then
					--显示选中框3
					_childUI["DLCMapInfo_JZ3_Selectbox_Finish"].handle._n:setVisible(true)
					_childUI["DLCMapInfo_JZ3_Selectbox_Finish"].handle._n:runAction(CCSequence:createWithTwoActions(CCScaleTo:create(0.05,0.52,0.52),CCScaleTo:create(0.05,0.5,0.5)))
		...
		_RemoveList[#_RemoveList+1] = "DLCMapInfo_JZ3_Btn"
		...
		_childUI["confirmBtn"] = hUI.button:new({
			...
			model = "misc/ugc/ugc_gobtn.png", --"misc/button_back.png",
			--label = {text = "生成随机迷宫", font = hVar.FONTC, size = 32, border = 1, x = 0, y = 2,}, --"确定"
			x = _frm.data.w/2,
			y = -_frm.data.h - 60,
			--scale = 1.08,
			scale = 1.0,
			...
			code = function()
				...
				if btnOk then
					...
					if (type(current_funCallbackOK) == "function") then
						current_funCallbackOK(_cur_index[hVar.ITEM_TYPE.WEAPON], _cur_index[hVar.ITEM_TYPE.BODY], _cur_wall_type, _cur_scroll_type)
					end
				end
			end,
		})
		...
		hApi.AddShader(_childUI["confirmBtn"].handle.s, "gray")
		--领取按钮的高亮状态1
		_childUI["confirmBtn"].childUI["image1"] = hUI.image:new({
			parent = _childUI["confirmBtn"].handle._n,
			model = "misc/ugc/ugc_gobtn3.png",
			x = 0,
			y = 0,
			z = 1,
			scale = 1.0,
		...
		--[[
		local act1 = CCScaleTo:create(0.2, 1.02)
		local act2 = CCScaleTo:create(0.2, 1.0)
		local a = CCArray:create()
		a:addObject(act1)
		a:addObject(act2)
		local sequence = CCSequence:create(a)
		_childUI["confirmBtn"].childUI["image1"].handle.s:runAction(CCRepeatForever:create(sequence))
		]]
		_childUI["confirmBtn"].childUI["image1"].handle._n:setVisible(false) --默认隐藏
		--领取按钮的高亮状态2
		_childUI["confirmBtn"].childUI["image2"] = hUI.image:new({
			parent = _childUI["confirmBtn"].handle._n,
			model = "MODEL_EFFECT:circlebutton",
			x = 0,
			y = 0,
			scale = 1.0,
		})
		_childUI["confirmBtn"].childUI["image2"].handle._n:setVisible(false) --默认隐藏
	end
	...
	getLeftRightOffset_RedEquip = function(equipType)
		...
		if DLCMapInfoBtn1 then
			...
			btn1_lx = btn1_cy + _itemOffX / 2 --第一个DLC地图面板最左侧的x坐标
			delta1_lx = btn1_lx - 210 - 80 --第一个DLC地图面板距离左侧边界的距离
			
			if (equipType == hVar.ITEM_TYPE.WEAPON) then --武器
				delta1_lx = btn1_lx + 165 + 20+40
			elseif (equipType == hVar.ITEM_TYPE.BODY) then --防具
				delta1_lx = btn1_lx + 165 + 20+40
			elseif (equipType == hVar.ITEM_TYPE.ORNAMENTS) then --宝物
				--
		...
		if DLCMapInfoBtnN then
			...
			btnN_rx = btnN_cy - _itemOffX / 2 --最后一个DLC地图面板最右侧的x坐标
			deltNa_rx = btnN_rx - 810 + 260 --最后一个DLC地图面板距离右侧边界的距离
			
			if (equipType == hVar.ITEM_TYPE.WEAPON) then --武器
				deltNa_rx = btnN_rx + 770 - 20+40
			elseif (equipType == hVar.ITEM_TYPE.BODY) then --防具
				deltNa_rx = btnN_rx + 770 - 20+40
			elseif (equipType == hVar.ITEM_TYPE.ORNAMENTS) then --宝物
				--
	...
	get_btn_state = function()
		if (_cur_index[hVar.ITEM_TYPE.WEAPON] > 0) and (_cur_index[hVar.ITEM_TYPE.BODY] > 0) and (_cur_wall_type > 0) and (_cur_scroll_type > 0) then
			return true
		else
			return false
		end
	...
	refresh_redequip_UI_scroll_loop_weapon = function()
		...
		if b_need_auto_fixing_redequip[hVar.ITEM_TYPE.WEAPON] then
			...
			elseif (draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] ~= 0) then --沿着当前的速度方向有惯性地运动一会
				--print("沿着当前的速度方向有惯性地运动一会", draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON])
				--不会选中神器
				selected_redequip_idx[hVar.ITEM_TYPE.WEAPON] = 0 --选中的神器索引
				--print("    ->   draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON]=", draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON])
				
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] > 0) then --朝右运动
					local speed = (draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON]) * 1.0 --系数
					friction_redequip[hVar.ITEM_TYPE.WEAPON] = friction_redequip[hVar.ITEM_TYPE.WEAPON] - 0.5
					draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] + friction_redequip[hVar.ITEM_TYPE.WEAPON] --衰减（正）
					
					if (draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] < 0) then
						draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = 0
					end
				...
				elseif (draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] < 0) then --朝左运动
					local speed = (draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON]) * 1.0 --系数
					friction_redequip[hVar.ITEM_TYPE.WEAPON] = friction_redequip[hVar.ITEM_TYPE.WEAPON] + 0.5
					draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] = draggle_speed_y_redequip[hVar.ITEM_TYPE.WEAPON] + friction_redequip[hVar.ITEM_TYPE.WEAPON] --衰减（负）
	...
	refresh_redequip_UI_scroll_loop_body = function()
		...
		if b_need_auto_fixing_redequip[hVar.ITEM_TYPE.BODY] then
			...
			elseif (draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] ~= 0) then --沿着当前的速度方向有惯性地运动一会
				--print("沿着当前的速度方向有惯性地运动一会", draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY])
				--不会选中神器
				selected_redequip_idx[hVar.ITEM_TYPE.BODY] = 0 --选中的神器索引
				--print("    ->   draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY]=", draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY])
				
				if (draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] > 0) then --朝右运动
					local speed = (draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY]) * 1.0 --系数
					friction_redequip[hVar.ITEM_TYPE.BODY] = friction_redequip[hVar.ITEM_TYPE.BODY] - 0.5
					draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] + friction_redequip[hVar.ITEM_TYPE.BODY] --衰减（正）
					
					if (draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] < 0) then
						draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = 0
					end
				...
				elseif (draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] < 0) then --朝左运动
					local speed = (draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY]) * 1.0 --系数
					friction_redequip[hVar.ITEM_TYPE.BODY] = friction_redequip[hVar.ITEM_TYPE.BODY] + 0.5
					draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] = draggle_speed_y_redequip[hVar.ITEM_TYPE.BODY] + friction_redequip[hVar.ITEM_TYPE.BODY] --衰减（负）
	...
	--监听收到红装卷轴可兑换的所有红装信息
	hGlobal.event:listen("LocalEvent_Phone_ShowGMResourceFrm_Vert", "GMResourceFrm_Vert", function(callbackOK, callbackCancel, bOpenImmediate)
		--print(_bCanCreate)
		...
		if _bCanCreate then
			...
			--移除事件监听：相册读取图片
			hGlobal.event:listen("LocalEvent_OnAlbumSelected", "OnAlbumSelected", on_receive_album_selected_event)
	...
	--监听关闭主界面事件
	hGlobal.event:listen("clearPhoneUserDefMapFrm_Vert", "GMResourceFrm_Vert", function()
		_bCanCreate = true
		
		--隐藏本界面
		if (_frm.data.show == 1) then --打开状态
			_frm:show(0)
			_exitFunc()
			--print("clearPhoneUserDefMapFrm_Vert")
		end
	end)
	
	--切换场景
	hGlobal.event:listen("LocalEvent_PlayerFocusWorld", "__SwitchGame_GMResourceFrm_Vert", function()
		_bCanCreate = true
		...
		if (_frm.data.show == 1) then --打开状态
			_frm:show(0)
			_exitFunc()
			--print("LocalEvent_PlayerFocusWorld")
		end
	end)
end

--打开用户自定义地图界面
hGlobal.event:listen("LocalEvent_Phone_ShowUserDefMapFrm", "ShowUserDefMapFrm", function(bOpenImmediate)
	--触发事件
	local callbackOK = function(backgroundId, avaterId, wallId, rollId)
		...
		tConfig.farobj = --地球远景层
		{
			...
			num = 1,
			scale = 1.0,
			--rollRatio = tUniverseConfig.farobj.rollRatio * rollId,
		}
		if (backgroundId == 7) then
			local g_localfilepath_top = string.sub(g_localfilepath, 1, #g_localfilepath-11)
			tConfig.farobj.img = {g_localfilepath_top .. COPY_PATH_BG .. current_bg_postfix,}
		end
		if (rollId == 1) then --快
			tConfig.farobj.rollRatio = 0.1
		elseif (rollId == 2) then --慢
			tConfig.farobj.rollRatio = 0.5
		elseif (rollId == 3) then --禁止
			tConfig.farobj.rollRatio = 1.0
		end
		tConfig.middleobj = tUniverseConfig.middleobj
		if (rollId == 1) then --快
			tConfig.middleobj.rollRatio = 0.16
		elseif (rollId == 2) then --慢
			tConfig.middleobj.rollRatio = 0.6
		elseif (rollId == 3) then --禁止
			tConfig.middleobj.rollRatio = 1.0
		end
		
		tConfig.nearobj = tUniverseConfig.nearobj
		if (rollId == 1) then --快
			tConfig.nearobj.rollRatio = 0.3
		elseif (rollId == 2) then --慢
			tConfig.nearobj.rollRatio = 0.8
		elseif (rollId == 3) then --禁止
			tConfig.nearobj.rollRatio = 1.0
		end
		...
		if (avaterId == hVar.RANDMAP_ROOM_AVATAR_INFO.RandRange+1) then
			local g_localfilepath_top = string.sub(g_localfilepath, 1, #g_localfilepath-11)
			tConfig.wallimg = g_localfilepath_top .. COPY_PATH_MAP .. current_map_postfix
		end
	...
	hGlobal.event:event("LocalEvent_Phone_ShowGMResourceFrm_Vert", callbackOK, callbackCancel, bOpenImmediate)
end)
...
hGlobal.UI.InitPlayerGMResourceFrm_Vert("include") --创建GM领取资源界面
hGlobal.event:event("LocalEvent_Phone_ShowUserDefMapFrm")
]]
